...
def read_dataset_config(dataset_path, datasets_toml_path="../datasets.toml"):
    ...
    if config is None:
        print(f"Warning: Dataset '{dataset_name}' not found in datasets.toml. Using defaults.")
        config = {
            "frequency": "D",
            "seasonality": 7,
            "test_split": 0.2
...
def get_model_config(environment_dict=None):
    ...
    seasonality = int(environment_dict.get("SEASONALITY", dataset_config.get("seasonality", 7)))
    
    # Handle test_split (default to 0.2 if not in config)
    test_split_env = environment_dict.get("TEST_SPLIT", dataset_config.get("test_split", 0.2))
