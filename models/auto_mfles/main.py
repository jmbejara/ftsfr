...
def forecast_auto_mfles(df, test_ratio, seasonality):
    """
    Fit AutoMFLES model and return MASE
    ...
    try:
        ...
        # setting test_size = 1 because we plan on producing T + 1
        # predictions to calculate MASE if trained upto T
        estimator = AutoMFLES(test_size = 1, season_length = seasonality)
        ...
        print(f"Error in AutoMFLES forecasting: {e}")
        return np.nan
...
if __name__ == "__main__":
    ...
    # Local forecasting
    print(f"Running AutoMFLES forecasting for {len(entities)} entities...")
    ...
    for entity in tqdm(entities):
        ...
        # Get MASE using AutoMFLES
        entity_mase = forecast_auto_mfles(entity_data, test_ratio, seasonality)
    ...
    results_file = OUTPUT_DIR / "raw_results" / f"auto_mfles_{dataset_name}_results.csv"
