...
Performs both local and global forecasting using a TFT. Reports both mean and
...
def forecast_tft(df, test_split, seasonality):
    """
    Fit TFT model and return MASE
    ...
    try:
        ...
        print(f"Error in TFT forecasting: {e}")
        return np.nan
...
if __name__ == "__main__":
    ...
    for entity in tqdm(entities):
        ...
        # Get MASE using TFT
        entity_mase = forecast_tft(entity_data, test_ratio, seasonality)
    ...
    global_mase = forecast_tft(proc_df, test_ratio, seasonality)
    ...
    results_df.to_csv(
        OUTPUT_DIR / "raw_results" / "tft_results.csv", index=False
    )
