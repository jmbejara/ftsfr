...
Performs both local and global forecasting using a N-HiTS. Reports both mean and
...
def forecast_nhits(df, test_split, seasonality):
    """
    Fit N-HiTS model and return MASE
    ...
    try:
        ...
        estimator = NHiTSModel(input_chunk_length = seasonality * 10,
        ...
        print(f"Error in N-HiTS forecasting: {e}")
        return np.nan
...
if __name__ == "__main__":
    ...
    print(f"Running N-HiTS forecasting for {len(entities)} entities...")
    ...
    for entity in tqdm(entities):
        ...
        # Get MASE using N-HiTS
        entity_mase = forecast_nhits(entity_data, test_ratio, seasonality)
    ...
    global_mase = forecast_nhits(proc_df,
                                       test_ratio,
                                       seasonality)

    # Printing and saving results

    print("\nN-HiTS Forecasting Results:")
    ...
    results_df.to_csv(OUTPUT_DIR / "raw_results" / "nhits_results.csv", index=False)
