...
def forecast_tide(df, test_split, seasonality):
    """
    Fit TiDE model and return mase
    ...
    try:
        ...
        print(f"Error in TiDE forecasting: {e}")
        return np.nan
...
if __name__ == "__main__":
    ...
    print(f"Running TiDE forecasting for {len(entities)} entities...")
    ...
    for entity in tqdm(entities):
        ...
        # Get MASE using TiDE
        entity_mase = forecast_tide(entity_data, test_ratio, seasonality)
    ...
    global_mase = forecast_tide(proc_df, test_ratio, seasonality)

    # Printing and saving results

    print("\nTiDE Forecasting Results:")
    ...
    results_df.to_csv(OUTPUT_DIR / "raw_results" / "tide_results.csv", index=False)
