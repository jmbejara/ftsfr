...
def get_deepar_forecasts_global(lag, df, test_ratio = 0.2, frequency = None, external_forecast_horizon = None):
    """
    Takes processed DataFrame, runs the training for a DeepAR model, and returns MASE
...
def get_deepar_forecasts_local(lag, df, frequency, external_forecast_horizon = None):
    """
    Takes processed DataFrame containing multiple or single time series, runs 
    the training for a separate DeepAR model on each series, and returns MASE
    ...
    print(f"Running DeepAR forecasting for {len(entities)} entities...")
    ...
    for entity in tqdm(entities):
        ...
        entity_mase = get_deepar_forecasts_global(lag=lag,
                                                df=entity_data,
...
if __name__ == "__main__":
    ...
    mase_values = get_deepar_forecasts_local(lag = 50, 
                                           df = proc_df,
                                           frequency = "1B",
    ...
    global_mase = get_deepar_forecasts_global(lag = 50,
                                            df = proc_df,
                                            test_ratio = 0.2,
                                            frequency = "1B",
                                            external_forecast_horizon = forecast_horizon)
    ...
    results_df.to_csv(OUTPUT_DIR / "raw_results" / "deepar_results.csv", index=False)