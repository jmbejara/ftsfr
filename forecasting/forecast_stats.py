...
def main():
    ...
    # Keep data in Polars format - StatsForecast supports Polars directly
    df = full_data
    ...
    sf = StatsForecast(
        models=models,
        freq=polars_frequency,  # Use Polars-compatible frequency
        n_jobs=-1,
    ...
    if model_name in avg_metrics:
        metrics_data = {
            ...
            "MASE": [avg_metrics[model_name]['MASE']],
            "MSE": [avg_metrics[model_name]['MSE']],
            "RMSE": [avg_metrics[model_name]['RMSE']],
            "R2oos": [avg_metrics[model_name]['R2oos']],
            "time_taken": [cv_time]
        }
