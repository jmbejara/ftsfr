...
def group_portfolios(bond_returns=None):
    ...
    grouped = (
        bond_returns.groupby(["month_end", "tau_group"])["tdretnua"].mean().reset_index()
    )
...
def calc_treasury_bond_returns(data_dir=DATA_DIR):
    ...
    """
    bond_returns = load_CRSP_treasury_consolidated(data_dir=data_dir, with_runness=False)
...
def calc_monthly_returns(daily_returns):
    ...
    monthly_returns = df.groupby(['month_end', 'kytreasno'])['tdretnua'].apply(
        lambda x: ((1 + x).prod() - 1)  # First get monthly return
    ).reset_index()
    
    other_cols = [col for col in df.columns if col not in ['caldt', 'tdretnua', 'month_end', 'kytreasno']]
    last_values = df.groupby(['month_end', 'kytreasno'])[other_cols].last().reset_index()
    
    # Merge monthly returns with other columns
    monthly_returns = monthly_returns.merge(
        last_values,
        on=['month_end', 'kytreasno'],
        how='left'
    )
...
def calc_returns(data_dir=DATA_DIR):
    daily_returns = load_CRSP_treasury_consolidated(data_dir=data_dir, with_runness=False)
