...
def generate_replication_gsci(data_dir=DATA_DIR):
    ...
    optimal_pairs_df1["GSCI Index"] = optimal_pairs_df1["Commodity_1"].str.replace("_PX_LAST_Return", "", regex=False)
...
def calc_lme_monthly_1mprice(
    lme_df, metal_map, date_col="index", price_func=None
):
...
def generate_replication_future_ticker():
    ...
    optimal_pairs_df2["Commodity_Name"] = optimal_pairs_df2["Commodity_2"].map(ticker_to_commodity)
...
def generate_replication_manual():
    ...
    optimal_pairs_df3["Commodity_Name"] = optimal_pairs_df3["Commodity_2"].map(ticker_to_commodity)
