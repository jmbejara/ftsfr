...
def get_data_requirements(frequency, test_size, seasonality=1):
    ...
    freq_configs = {
        ...
        "train_ratio": 1.0,
        "test_ratio": 0.5,
        "min_test_completion_ratio": 0.75,
    ...
    completion_ratio = config.get("min_test_completion_ratio", 0.5)
    min_test_obs = max(
        min(test_size, max(config["base_min_test"], adaptive_test_floor)),
        math.ceil(test_size * completion_ratio),
    )
    ...
    return {
        ...
        "min_test_completion_ratio": completion_ratio,
    }
