# %%
"""
# CDS-Bond Basis Spread Forecasting Walkthrough

This walkthrough demonstrates forecasting the High Yield CDS-Bond basis spread
using StatsForecast and NeuralForecast. The CDS-Bond basis represents the arbitrage
spread between Credit Default Swap (CDS) premiums and bond yields.

This example helps us understand:
- How different statistical models perform on credit arbitrage spreads
...
The CDS-Bond basis dataset contains arbitrage spreads aggregated by credit rating:
- High Yield: Below investment grade (BB+ and lower)
- Investment Grade: BBB- and higher

For this walkthrough, we'll focus on High Yield to understand
how well different models can forecast this credit arbitrage spread.
"""
...
Load the CDS-Bond basis data and filter to the High Yield series.
...
data_path = REPO_ROOT / "_data/formatted/cds_bond_basis/ftsfr_CDS_bond_basis_aggregated.parquet"
...
# %%
# Load data and filter to High Yield
df_raw = pl.read_parquet(data_path)
df_hy = df_raw.filter(pl.col("unique_id") == "High Yield")

# Convert to pandas for StatsForecast
Y_df = df_hy.to_pandas()

# Ensure datetime format
Y_df['ds'] = pd.to_datetime(Y_df['ds'])

# Data is already monthly, no resampling needed
# Handle missing values if any (forward fill then backward fill if needed)
Y_df['y'] = Y_df['y'].ffill().bfill()

print(f"Loaded {len(Y_df)} observations of High Yield CDS-Bond basis spread (monthly)")
...
Plot the complete High Yield CDS-Bond basis spread to understand its behavior.
"""
...
3. **Theta** – decomposes the series with an annual seasonality component.

**Auto-tuned neural networks (Optuna search):**
4. **Auto DeepAR**
5. **Auto DLinear**
6. **Auto Vanilla Transformer**
7. **Auto TiDE**
...
credit basis spreads (fiscal year-end flows, credit cycle seasonality).
"""
...
# CDS-Bond basis spreads often exhibit annual patterns due to:
# - Fiscal year-end effects
# - Credit market cycles
# - Seasonal corporate issuance patterns
season_length = 12
...
auto_logs_dir = (REPO_ROOT / "_output/forecasting/logs/example_forecasts_cds_bond").resolve()
...
# %%
nf = NeuralForecast(models=neural_models, freq="ME")
nf.fit(df=train_df)
...
Plot all model predictions alongside the actual test observations.
"""
...
ax.set_title("All Model Forecasts vs Actuals (Last 5+ Years) - High Yield CDS-Bond Basis")
ax.set_xlabel("Date")
ax.set_ylabel("Basis Spread")
ax.legend(ncol=2, fontsize="small", loc="best")
ax.grid(True, alpha=0.3)
plt.tight_layout()
save_and_show_plot("example_forecasts_cds_bond_all_models.png")
...
# nf_cv = NeuralForecast(models=neural_cv_models, freq="ME")
#
...
#
# plt.tight_layout()
# save_and_show_plot("example_forecasts_cds_bond_model_rankings.png")
#
# # %%
# """
# ## Identify Best Model
...
# best_forecast = combined_forecasts_df[combined_forecasts_df["unique_id"] == "High Yield"]
#
...
# plt.tight_layout()
# save_and_show_plot("example_forecasts_cds_bond_best_model.png")
...
1. **Best Model**: The model with the lowest MASE performs best at forecasting
   the High Yield CDS-Bond basis spread.

2. **Model Comparison**:
   - The auto neural family (DeepAR, DLinear, TiDE, Transformer) explores rich function spaces
...
3. **Why This Matters**:
   - CDS-Bond basis spreads represent credit arbitrage opportunities
   - Understanding which models work helps identify when spreads deviate from expected patterns
   - Poor model performance might indicate structural breaks or credit regime changes

### Next Steps:

- Experiment with different forecast horizons
- Try models with exogenous variables (volatility, credit spreads, liquidity measures)
- Compare performance across credit rating categories (High Yield vs Investment Grade)
- Investigate periods where all models perform poorly (potential credit regime changes)
"""
...
# for model in all_model_cols[:3]:  # Show first 3 models as example
