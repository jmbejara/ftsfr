...
FORECAST_DIR = OUTPUT_DIR / "forecasting"  # New forecasting output directory
        ...
        if isinstance(module_config, dict):
            ...
            for key, value in module_config.items():
                if isinstance(value, dict) and key.startswith("ftsfr_"):
                    ...
                    table_name = value.get(
                        "table_name", value.get("short_name", dataset_name)
                    )
...
def filter_results_by_active_datasets(results_df):
    ...
    initial_count = len(results_df)
    filtered_results = results_df[
        results_df["dataset_name"].isin(active_datasets)
    ].copy()
    filtered_count = len(filtered_results)

    if filtered_count < initial_count:
        removed_count = initial_count - filtered_count
        removed_datasets = set(results_df["dataset_name"].unique()) - set(
            active_datasets
        )
        print(
            f"Filtered out {removed_count} results from inactive datasets: {sorted(removed_datasets)}"
        )
...
def filter_quality_results(results_df):
    ...
    if initial_count > 0:
        print(
            f"Final results: {len(quality_results)} rows ({len(quality_results) / initial_count * 100:.1f}% retained)"
        )
    else:
        print(
            f"Final results: {len(quality_results)} rows (N/A% retained - no initial data)"
        )
...
def create_quality_summary():
    ...
    error_metrics_dir = FORECAST_DIR / "error_metrics"
        ...
        for dataset in all_datasets:
            ...
            if not csv_path.exists():
                ...
                result_row = results_df[
                    (results_df["model_name"] == models_config[model]["display_name"])
                ...
                if result_row.empty:
                    ...
                    error_columns = ["MASE", "RMSE"]
                    available_columns = [
                        col for col in error_columns if col in result_row.columns
                    ]

                    has_nan = (
                        result_row[available_columns].isna().any().any()
                        if available_columns
                        else False
                    )
                    has_zero = (
                        (result_row[available_columns] == 0).any().any()
                        if available_columns
                        else False
                    )
    ...
    quality_csv = FORECAST_DIR / "quality_summary_detailed.csv"
    ...
    # Save pivot table
    quality_pivot_csv = FORECAST_DIR / "quality_summary_pivot.csv"
    ...
    quality_tex = FORECAST_DIR / "quality_summary.tex"
    ...
    quality_tabular_tex = FORECAST_DIR / "quality_summary_tabular.tex"
    with open(quality_tabular_tex, "w") as f:
        f.write(
            f"% Quality Summary - tabular content only\n% Generated automatically by create_results_tables2.py\n{quality_tabular}"
        )
...
def create_failure_analysis(quality_df):
    ...
    model_analysis.columns = [
        "Total_Datasets",
        "Success_Count",
        "Quality_Issues_Count",
        "No_CSV_Count",
        "CSV_But_No_Results_Count",
    ]

    # Calculate success rates
    model_analysis["Success_Rate"] = (
        model_analysis["Success_Count"] / model_analysis["Total_Datasets"] * 100
    ).round(1)
    ...
    # Save model analysis
    model_csv = FORECAST_DIR / "model_failure_analysis.csv"
    ...
    model_tex = FORECAST_DIR / "model_failure_analysis.tex"
    ...
    model_tabular_tex = FORECAST_DIR / "model_failure_analysis_tabular.tex"
    with open(model_tabular_tex, "w") as f:
        f.write(
            f"% Model Failure Analysis - tabular content only\n% Generated automatically by create_results_tables2.py\n{model_tabular}"
        )
    print(f"Saved model failure analysis tabular (LaTeX) to: {model_tabular_tex}")

    # Dataset failure analysis
    dataset_analysis = (
        quality_df.groupby("dataset_name")
        .agg(
            {
                "Status": [
                    "count",
    ...
    # Calculate success rates
    dataset_analysis["Success_Rate"] = (
        dataset_analysis["Success_Count"] / dataset_analysis["Total_Models"] * 100
    ).round(1)
    ...
    dataset_csv = FORECAST_DIR / "dataset_failure_analysis.csv"
    ...
    dataset_tex = FORECAST_DIR / "dataset_failure_analysis.tex"
    ...
    dataset_tabular_tex = FORECAST_DIR / "dataset_failure_analysis_tabular.tex"
    with open(dataset_tabular_tex, "w") as f:
        f.write(
            f"% Dataset Failure Analysis - tabular content only\n% Generated automatically by create_results_tables2.py\n{dataset_tabular}"
        )
...
def create_mase_pivot_table():
    ...
    print(
        f"Created pivot table with {len(mase_pivot)} datasets and {len(mase_pivot.columns)} models"
    )
    ...
    if model_order:
        ...
        existing_ordered_columns = [
            col for col in ordered_display_names if col in mase_pivot.columns
        ]
        # Add any columns not in the config (shouldn't happen, but safety check)
        remaining_columns = [
            col for col in mase_pivot.columns if col not in existing_ordered_columns
        ]
    ...
    mase_pivot = apply_grouped_dataset_ordering(
        mase_pivot, dataset_groups, dataset_table_names
    )

    # Save as CSV for inspection
    csv_file = FORECAST_DIR / "mase_pivot_table.csv"
    ...
    paper_csv_file = PAPER_DIR / "mase_pivot_table.csv"
    paper_csv_file.parent.mkdir(parents=True, exist_ok=True)
    mase_pivot.to_csv(paper_csv_file)
    print(f"Saved MASE pivot table (CSV) to: {paper_csv_file}")

    # Convert to LaTeX table
    latex_output = create_latex_table(
        mase_pivot, "MASE Results by Dataset and Model", "tab:mase_results"
    )
    ...
    paper_tex_file = PAPER_DIR / "mase_pivot_table.tex"
    with open(paper_tex_file, "w") as f:
        f.write(latex_output)
    print(f"Saved MASE pivot table (LaTeX) to: {paper_tex_file}")

    # Save tabular-only version to paper directory
    paper_tabular_file = PAPER_DIR / "mase_pivot_tabular.tex"
    with open(paper_tabular_file, "w") as f:
        f.write(latex_tabular_output)
    print(f"Saved MASE pivot tabular (LaTeX) to: {paper_tabular_file}")

    # Save as .tex file
    tex_file = FORECAST_DIR / "mase_pivot_table.tex"
    with open(tex_file, "w") as f:
        f.write(latex_output)
    print(f"Saved MASE pivot table (LaTeX) to: {tex_file}")

    # Save tabular-only version
    tabular_file = FORECAST_DIR / "mase_pivot_tabular.tex"
...
def create_rmse_pivot_table():
    ...
    print(
        f"Created pivot table with {len(rmse_pivot)} datasets and {len(rmse_pivot.columns)} models"
    )
    ...
    if model_order:
        ...
        # Filter to only include columns that actually exist in the pivot table
        existing_ordered_columns = [
            col for col in ordered_display_names if col in rmse_pivot.columns
        ]
        # Add any columns not in the config (shouldn't happen, but safety check)
        remaining_columns = [
            col for col in rmse_pivot.columns if col not in existing_ordered_columns
        ]
        final_column_order = existing_ordered_columns + remaining_columns
        # Reorder the columns
    ...
    # Apply grouped dataset ordering (same as LaTeX tables)
    dataset_groups, dataset_table_names = load_dataset_groups_and_names()
    rmse_pivot = apply_grouped_dataset_ordering(
        rmse_pivot, dataset_groups, dataset_table_names
    )

    # Save as CSV for inspection
    csv_file = FORECAST_DIR / "rmse_pivot_table.csv"
    rmse_pivot.to_csv(csv_file)
    print(f"Saved RMSE pivot table (CSV) to: {csv_file}")

    # Save also to paper directory for organized access
    paper_csv_file = PAPER_DIR / "rmse_pivot_table.csv"
    paper_csv_file.parent.mkdir(parents=True, exist_ok=True)
    rmse_pivot.to_csv(paper_csv_file)
    print(f"Saved RMSE pivot table (CSV) to: {paper_csv_file}")

    # Convert to LaTeX table
    latex_output = create_latex_table(
        rmse_pivot, "RMSE Results by Dataset and Model", "tab:rmse_results"
    )
    latex_tabular_output = create_latex_tabular_only(
        rmse_pivot, "RMSE Results by Dataset and Model", "tab:rmse_results"
    )
    ...
    # Save as .tex file
    tex_file = FORECAST_DIR / "rmse_pivot_table.tex"
    with open(tex_file, "w") as f:
        f.write(latex_output)
    print(f"Saved RMSE pivot table (LaTeX) to: {tex_file}")

    # Save tabular-only version
    tabular_file = FORECAST_DIR / "rmse_pivot_tabular.tex"
    with open(tabular_file, "w") as f:
        f.write(latex_tabular_output)
    print(f"Saved RMSE pivot tabular (LaTeX) to: {tabular_file}")

    # Save LaTeX version to paper directory as well
    paper_tex_file = PAPER_DIR / "rmse_pivot_table.tex"
    with open(paper_tex_file, "w") as f:
        f.write(latex_output)
    print(f"Saved RMSE pivot table (LaTeX) to: {paper_tex_file}")

    # Save tabular-only version to paper directory
    paper_tabular_file = PAPER_DIR / "rmse_pivot_tabular.tex"
    with open(paper_tabular_file, "w") as f:
        f.write(latex_tabular_output)
    print(f"Saved RMSE pivot tabular (LaTeX) to: {paper_tabular_file}")
...
def create_r2oos_pivot_table():
    ...
    print(
        f"Created pivot table with {len(r2oos_pivot)} datasets and {len(r2oos_pivot.columns)} models"
    )
    ...
    if model_order:
        ...
        # Filter to only include columns that actually exist in the pivot table
        existing_ordered_columns = [
            col for col in ordered_display_names if col in r2oos_pivot.columns
        ]
        # Add any columns not in the config (shouldn't happen, but safety check)
        remaining_columns = [
            col for col in r2oos_pivot.columns if col not in existing_ordered_columns
        ]
        final_column_order = existing_ordered_columns + remaining_columns
        # Reorder the columns
    ...
    # Apply grouped dataset ordering (same as LaTeX tables)
    dataset_groups, dataset_table_names = load_dataset_groups_and_names()
    r2oos_pivot = apply_grouped_dataset_ordering(
        r2oos_pivot, dataset_groups, dataset_table_names
    )

    # Save as CSV for inspection
    csv_file = FORECAST_DIR / "r2oos_pivot_table.csv"
    r2oos_pivot.to_csv(csv_file)
    print(f"Saved R2oos pivot table (CSV) to: {csv_file}")

    # Save also to paper directory for organized access
    paper_csv_file = PAPER_DIR / "r2oos_pivot_table.csv"
    paper_csv_file.parent.mkdir(parents=True, exist_ok=True)
    r2oos_pivot.to_csv(paper_csv_file)
    print(f"Saved R2oos pivot table (CSV) to: {paper_csv_file}")

    # Convert to LaTeX table
    latex_output = create_latex_table(
        r2oos_pivot, "R2oos Results by Dataset and Model", "tab:r2oos_results"
    )
    latex_tabular_output = create_latex_tabular_only(
        r2oos_pivot, "R2oos Results by Dataset and Model", "tab:r2oos_results"
    )
    ...
    # Save as .tex file
    tex_file = FORECAST_DIR / "r2oos_pivot_table.tex"
    with open(tex_file, "w") as f:
        f.write(latex_output)
    print(f"Saved R2oos pivot table (LaTeX) to: {tex_file}")

    # Save tabular-only version
    tabular_file = FORECAST_DIR / "r2oos_pivot_tabular.tex"
    with open(tabular_file, "w") as f:
        f.write(latex_tabular_output)
    print(f"Saved R2oos pivot tabular (LaTeX) to: {tabular_file}")

    # Save LaTeX version to paper directory as well
    paper_tex_file = PAPER_DIR / "r2oos_pivot_table.tex"
    with open(paper_tex_file, "w") as f:
        f.write(latex_output)
    print(f"Saved R2oos pivot table (LaTeX) to: {paper_tex_file}")

    # Save tabular-only version to paper directory
    paper_tabular_file = PAPER_DIR / "r2oos_pivot_tabular.tex"
    with open(paper_tabular_file, "w") as f:
        f.write(latex_tabular_output)
    print(f"Saved R2oos pivot tabular (LaTeX) to: {paper_tabular_file}")
    ...
    if not valid_data.empty:
        ...
        print(
            f"Average models with positive R2oos per dataset: {positive_r2_count.mean():.1f}"
        )
        ...
        print(
            f"Best overall model: {best_model} (avg R2oos: {overall_performance[best_model]:.3f})"
        )
        print(
            f"Worst overall model: {worst_model} (avg R2oos: {overall_performance[worst_model]:.3f})"
        )
...
def create_median_mase_summary_table():
    ...
    if model_order:
        ...
        display_name_order = {
            model["display_name"]: i for i, model in enumerate(model_order)
        }

        # Add ordering column, defaulting to 999 for models not in config
        model_summary["Order"] = model_summary.index.map(
            lambda x: display_name_order.get(x, 999)
        )

        # Sort by order first, then by median MASE within each group
        model_summary = model_summary.sort_values(["Order", "Median_MASE"]).drop(
            "Order", axis=1
        )
    ...
    # Save as CSV for inspection
    csv_file = FORECAST_DIR / "median_mase_summary.csv"
    model_summary.to_csv(csv_file)
    print(f"Saved median MASE summary (CSV) to: {csv_file}")

    # Save also to paper directory for organized access
    paper_csv_file = PAPER_DIR / "median_mase_summary.csv"
    paper_csv_file.parent.mkdir(parents=True, exist_ok=True)
    model_summary.to_csv(paper_csv_file)
    print(f"Saved median MASE summary (CSV) to: {paper_csv_file}")
    ...
    model_summary_latex.index = model_summary_latex.index.map(
    ...
    # Save as .tex file
    tex_file = FORECAST_DIR / "median_mase_summary.tex"
    with open(tex_file, "w") as f:
        f.write(latex_output)
    ...
    # Save LaTeX version to paper directory as well
    paper_tex_file = PAPER_DIR / "median_mase_summary.tex"
    with open(paper_tex_file, "w") as f:
        f.write(latex_output)
    print(f"Saved median MASE summary (LaTeX) to: {paper_tex_file}")

    # Save tabular-only version to paper directory
    paper_tabular_file = PAPER_DIR / "median_mase_summary_tabular.tex"
    with open(paper_tabular_file, "w") as f:
        f.write(
            f"% Median MASE Summary - tabular content only\n% Generated automatically by create_results_tables.py\n{tabular_only}"
        )
    print(f"Saved median MASE summary tabular (LaTeX) to: {paper_tabular_file}")

    # Print summary statistics
    print("\nMedian MASE Summary Statistics:")
    print(f"Number of models: {len(model_summary)}")
    print(
        f"Best performing model (lowest median MASE): {model_summary.index[0]} ({model_summary['Median_MASE'].iloc[0]:.3f})"
    )
    print(
        f"Worst performing model (highest median MASE): {model_summary.index[-1]} ({model_summary['Median_MASE'].iloc[-1]:.3f})"
    )
    ...
    for i, (model, row) in enumerate(model_summary.head().iterrows()):
        print(
            f"  {i + 1}. {model}: {row['Median_MASE']:.3f} (across {int(row['N_Datasets'])} datasets)"
        )
...
def create_sectioned_latex_table(df, caption, label="tab:mase_results"):
    ...
    for dataset in df_original.index:
        ...
        if not numeric_values.isna().all():  # If we have any valid numbers
            if is_r2oos:
                ...
                best_model_cols = numeric_values[
    ...
    # Create header (escape underscores for LaTeX)
    escaped_columns = [col.replace("_", "\\_") for col in df_formatted.columns]
    ...
    for group in group_order:
        ...
        # Add group header
        tabular_content += (
            f"\\multicolumn{{{num_cols + 1}}}{{l}}{{{group_labels[group]}}} \\\\\n"
        )
...
def extract_tabular_content(latex_output):
    ...
    tabular_match = re.search(
        r"\\begin\{tabular\}.*?\\end\{tabular\}", latex_output, re.DOTALL
    )
    if tabular_match:
...
def create_latex_tabular_only(
    df, caption, label="tab:mase_results", use_table_names=True
):
...
def create_heatmap_plots():
    ...
    dataset_groups, dataset_table_names = load_dataset_groups_and_names()
    results["Dataset_Short"] = (
        results["dataset_name"].map(dataset_table_names).fillna(results["dataset_name"])
    )
    ...
    for metric_col, metric_short, filename, metric_long in error_metrics:
        ...
        if model_order:
            ...
            # Filter to only include columns that actually exist in the pivot table
            existing_ordered_columns = [
                col for col in ordered_display_names if col in pivot_data.columns
            ]
            # Add any columns not in the config (shouldn't happen, but safety check)
            remaining_columns = [
                col for col in pivot_data.columns if col not in existing_ordered_columns
            ]
            final_column_order = existing_ordered_columns + remaining_columns
            # Reorder the columns
        ...
        # Apply grouped dataset ordering (same as LaTeX tables)
        pivot_data = apply_grouped_dataset_ordering(
            pivot_data, dataset_groups, dataset_table_names
        )
        ...
        if metric_col == "Relative_MASE":
            ...
            if not heatmap_data.empty:
                ...
                max_abs = min(
                    max_abs, 1.0
                )  # Most R2oos values should be between -1 and 1
            ...
            if not heatmap_data.empty:
                ...
                vmax_val = heatmap_data.quantile(
                    0.90
                ).max()  # Changed from 0.95 to 0.90
        ...
        annot_data = pd.DataFrame(
            index=heatmap_data.index, columns=heatmap_data.columns, dtype=object
        )
        ...
        plt.title(
            f"{metric_long} by Dataset and Model\n{title_subtitle}",
            fontsize=20,
            fontweight="bold",
            pad=25,
        )
        ...
        output_file = PAPER_DIR / filename
        plt.savefig(
            output_file,
            dpi=600,
            bbox_inches="tight",
...
def create_summary_statistics():
    ...
    model_summary = (
        ...
        .agg(
            {"MASE": ["count", "mean", "std", "min", "max"], "dataset_name": "nunique"}
        )
        .round(3)
    )
    ...
    if "R2oos" in results.columns:
        r2oos_summary = (
            results.groupby("model_name")["R2oos"].agg(["mean", "std"]).round(3)
        )
    ...
    if "Forecast_Time_seconds" in results.columns:
        time_summary = (
            results.groupby("model_name")["Forecast_Time_seconds"]
            .agg(["mean", "sum"])
            .round(2)
        )
    ...
    # Save summary
    summary_file = FORECAST_DIR / "model_summary_statistics.csv"
    ...
    summary_tex_file = FORECAST_DIR / "model_summary_statistics.tex"
    ...
    summary_tabular_tex_file = FORECAST_DIR / "model_summary_statistics_tabular.tex"
    with open(summary_tabular_tex_file, "w") as f:
        f.write(
            f"% Model Summary Statistics - tabular content only\n% Generated automatically by create_results_tables2.py\n{summary_tabular}"
        )
    print(
        f"Saved model summary statistics tabular (LaTeX) to: {summary_tabular_tex_file}"
    )
...
def create_slurm_job_summary():
    ...
    # Save summary
    summary_csv = FORECAST_DIR / "slurm_job_summary.csv"
...
if __name__ == "__main__":
    ...
    print(f"  - {FORECAST_DIR / 'quality_summary_detailed.csv'}")
    print(f"  - {FORECAST_DIR / 'quality_summary_pivot.csv'}")
    print(f"  - {FORECAST_DIR / 'quality_summary.tex'}")
    print(f"  - {FORECAST_DIR / 'model_failure_analysis.csv'}")
    print(f"  - {FORECAST_DIR / 'model_failure_analysis.tex'}")
    print(f"  - {FORECAST_DIR / 'dataset_failure_analysis.csv'}")
    print(f"  - {FORECAST_DIR / 'dataset_failure_analysis.tex'}")
    print("Main Tables (with quality filtering):")
    print(f"  - {FORECAST_DIR / 'mase_pivot_table.csv'}")
    print(f"  - {FORECAST_DIR / 'mase_pivot_table.tex'}")
    print(f"  - {FORECAST_DIR / 'rmse_pivot_table.csv'}")
    print(f"  - {FORECAST_DIR / 'rmse_pivot_table.tex'}")
    ...
    print("Additional Output (paper directory):")
    print(f"  - {PAPER_DIR / 'mase_pivot_table.csv'}")
    print(f"  - {PAPER_DIR / 'mase_pivot_table.tex'}")
    print(f"  - {PAPER_DIR / 'rmse_pivot_table.csv'}")
    print(f"  - {PAPER_DIR / 'rmse_pivot_table.tex'}")
    if r2oos_table is not None:
