...
def filter_quality_results(results_df):
    ...
    # Check only the standardized columns g_mase and g_rmse
    error_columns = ['g_mase', 'g_rmse']
    available_columns = [col for col in error_columns if col in results_df.columns]
    
    if not available_columns:
        print("Warning: No g_mase or g_rmse columns found for quality filtering")
        return results_df
...
def create_quality_summary():
    ...
    # Get all expected model x dataset combinations from the configs
    import toml
    
    # Load models config
    models_config_path = Path(__file__).parent.parent / "models" / "models_config.toml"
    ...
    # Get datasets from existing results or scan error_metrics directory
    ...
    # Scan for all datasets across all model directories
    all_datasets = set()
    for model_dir in error_metrics_dir.iterdir():
        if model_dir.is_dir():
    ...
    results_file = OUTPUT_DIR / "results_all.csv"
    ...
    for model in all_models:
        for dataset in all_datasets:
            ...
            csv_path = error_metrics_dir / model / f"{dataset}.csv"
            ...
            if not csv_path.exists():
                ...
                if result_row.empty:
                    ...
                    # Check for quality issues in g_mase and g_rmse columns only
                    error_columns = ['g_mase', 'g_rmse']
    ...
    quality_csv = OUTPUT_DIR / "quality_summary_detailed.csv"
    ...
    # Save pivot table
    quality_pivot_csv = OUTPUT_DIR / "quality_summary_pivot.csv"
    ...
    quality_tex = OUTPUT_DIR / "quality_summary.tex"
...
def create_failure_analysis(quality_df):
    ...
    # Save model analysis
    model_csv = OUTPUT_DIR / "model_failure_analysis.csv"
    ...
    model_tex = OUTPUT_DIR / "model_failure_analysis.tex"
    ...
    dataset_csv = OUTPUT_DIR / "dataset_failure_analysis.csv"
    ...
    dataset_tex = OUTPUT_DIR / "dataset_failure_analysis.tex"
...
def create_mase_pivot_table():
    ...
    if not results_file.exists():
        ...
        print("Please run the assemble_results task first")
        sys.exit(1)
    ...
    if 'g_mase' not in results.columns:
        print("Error: g_mase column not found in results")
        print(f"Available columns: {list(results.columns)}")
        sys.exit(1)
    
    # Create pivot table: models as rows, datasets as columns, values as g_mase
    ...
    csv_file = OUTPUT_DIR / "mase_pivot_table.csv"
    ...
    # Save as .tex file
    tex_file = OUTPUT_DIR / "mase_pivot_table.tex"
...
def create_summary_statistics():
    ...
    model_summary = results.groupby('model').agg({
        'g_mase': ['count', 'mean', 'std', 'min', 'max'],
        'dataset': 'nunique'
    }).round(3)
    
    # Flatten column names
    model_summary.columns = ['N_Datasets', 'g_mase_Avg', 'g_mase_Std', 'g_mase_Min', 'g_mase_Max', 'N_Datasets_Check']
    model_summary = model_summary.drop('N_Datasets_Check', axis=1)
    
    # Save summary
    summary_file = OUTPUT_DIR / "model_summary_statistics.csv"
    ...
    summary_tex_file = OUTPUT_DIR / "model_summary_statistics.tex"
...
if __name__ == "__main__":
    print("Creating results tables with quality analysis...")
    ...
    print(f"  - {OUTPUT_DIR / 'quality_summary_detailed.csv'}")
    print(f"  - {OUTPUT_DIR / 'quality_summary_pivot.csv'}")
    print(f"  - {OUTPUT_DIR / 'quality_summary.tex'}")
    print(f"  - {OUTPUT_DIR / 'model_failure_analysis.csv'}")
    print(f"  - {OUTPUT_DIR / 'model_failure_analysis.tex'}")
    print(f"  - {OUTPUT_DIR / 'dataset_failure_analysis.csv'}")
    print(f"  - {OUTPUT_DIR / 'dataset_failure_analysis.tex'}")
    print("Original Tables (now with quality filtering):")
    print(f"  - {OUTPUT_DIR / 'mase_pivot_table.csv'}")
    print(f"  - {OUTPUT_DIR / 'mase_pivot_table.tex'}")
    print(f"  - {OUTPUT_DIR / 'model_summary_statistics.csv'}")
    print(f"  - {OUTPUT_DIR / 'model_summary_statistics.tex'}")