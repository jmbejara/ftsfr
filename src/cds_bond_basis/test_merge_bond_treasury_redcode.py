...
def test_merge_treasuries_into_bonds():
    ...
    assert set(merged_df.columns) == expected_cols, (
        f"Expected columns: {expected_cols}, but got {set(merged_df.columns)}"
    )

    expected_ylds = {
        datetime.datetime(2030, 1, 1, 0, 0): 0.03,
        datetime.datetime(2030, 6, 1, 0, 0): 0.032,
    }
...
def test_merge_red_code_into_bond_treas():
    ...
    for _, row in merged_df.iterrows():
        assert row["issuer_cusip"] == row["cusip"][:6], (
            f"Issuer CUSIP mismatch: {row['issuer_cusip']} != {row['cusip'][:6]}"
        )
