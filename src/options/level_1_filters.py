...
def identical_but_price_filter(df):
    ...
    is_neighbor = df.set_index(group_keys).index.isin(
        df_dupes.set_index(group_keys).index
    )
    ...
    df_joined = pd.merge(
        df_dupes, nearest[group_keys + ["mon_IV"]], on=group_keys, how="inner"
    )
...
def execute_appendixBfilter_level1(start=START_DATE_01, end=END_DATE_01):
    ...
    df = pull_option_data.load_all_optm_data(
        data_dir=DATA_DIR, wrds_username=WRDS_USERNAME, startDate=start, endDate=end
    )
    ...
    save_path = DATA_DIR.joinpath(
        f"intermediate/data_{start[:7]}_{end[:7]}_L1filter.parquet"
    )
...
if __name__ == "__main__":
    ...
    t1 = time.time() - t0
    print(
        f"Took {t1} to complete Level B1 Analysis on {START_DATE_01} -> {END_DATE_01} \n"
    )
    ...
    t1 = time.time() - t0
    print(
        f"Took {t1} to complete Level B1 Analysis on {START_DATE_02} -> {END_DATE_02} \n"
    )
