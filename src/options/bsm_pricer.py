...
def calc_implied_volatility(
    ...
    if method == "quasi_newton":
        ...
        return {"quasi_newton": iv_qn, "newton_raphson": iv_nr, "binary_search": iv_bs}
...
def check_nearest_iv(iv_results: dict, check_iv: float):
    ...
    nearest_method = min(
        [
            (
                iv_method,
                abs(calculated_iv - check_iv)
                if not (np.isnan(abs(calculated_iv - check_iv)))
                else 1e11,
            )
            for iv_method, calculated_iv in iv_results.items()
        ],
        key=lambda x: x[1],
    )[0]
...
def iv_quasi_newton(
    ...
    if result.success:
        ...
        print(
            ValueError(
                f">> Optimization was not successful. S={S}, K={K}, T={T}, r={r}, option_type={option_type}, market_price={market_price}"
            )
        )
        return np.nan
...
def iv_binary_search(
    ...
    while sigma_low < sigma_high:
        ...
        price_mid = (
            european_call_price(S, K, T, r, sigma_mid)
            if option_type == "call"
            else european_put_price(S, K, T, r, sigma_mid)
        )
...
if __name__ == "__main__":
    ...
    print(
        ...
        european_call_price(S=S, K=K, r=r, T=T, sigma=sigma),
    )
    print(
        "European put option price:",
        european_put_price(S=S, K=K, r=r, T=T, sigma=sigma),
    )
    ...
    iv_nr = iv_newton_raphson(
        option_market_price, S, K, T, r, option_type="call", sigma_est=0.1
    )
    iv_bs = iv_binary_search(
        option_market_price,
        S,
    ...
    iv_all = calc_implied_volatility(
        option_market_price,
        S,
        K,
        T,
