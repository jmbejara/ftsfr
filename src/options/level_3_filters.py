...
def build_l2_data_chart(l2_data, date_range):
    ...
    ax[1, 0].scatter(
        x=l2_data.xs("C", level="cp_flag")["moneyness"],
        y=np.exp(l2_data.xs("C", level="cp_flag")["log_iv"]),
        color="blue",
        alpha=0.1,
        label="IV",
    )
    ...
    ax[1, 1].scatter(
        x=l2_data.xs("P", level="cp_flag")["moneyness"],
        y=np.exp(l2_data.xs("P", level="cp_flag")["log_iv"]),
        color="red",
        alpha=0.1,
        label="IV",
    )
    ...
    if len(nan_iv_calls) == 0 and len(nan_iv_puts) == 0:
        ...
        nan_percentage = l2_data.groupby(["date", "cp_flag"])["IV"].apply(
            lambda x: (x.isna().sum() / len(x)) * 100
        )

        # calls only
        nan_percentage_calls = nan_percentage[
            nan_percentage.index.get_level_values(1) == "C"
        ]
        ax[1, 2].scatter(
            x=nan_percentage_calls.index.get_level_values(0),
            y=nan_percentage_calls.values,
            color="blue",
            alpha=0.1,
            s=10,
            label="Calls",
        ...
        # puts only
        nan_percentage_puts = nan_percentage[
            nan_percentage.index.get_level_values(1) == "P"
        ]
        ax[1, 2].scatter(
            x=nan_percentage_puts.index.get_level_values(0),
            y=nan_percentage_puts.values,
            color="red",
            alpha=0.1,
            s=10,
            label="Puts",
...
def build_l2_fitted_iv_chart(l2_data, date_range):
    ...
    ax[0, 2].scatter(
        x=l2_data["log_iv"], y=l2_data["fitted_iv"], color="darkblue", alpha=0.1
    )
    ...
    # Add 45-deg line
    ax[0, 2].plot(
        [min(l2_data["log_iv"]), max(l2_data["log_iv"])],
        [min(l2_data["log_iv"]), max(l2_data["log_iv"])],
        color="red",
        linestyle="--",
    )
    ...
    plt.suptitle(
        f"Level 2 Filtered Data with Fitted IVs: {date_range.replace('_', ' to ')}"
    )
    plt.tight_layout()
    # plt.show()
...
def build_l3_data_iv_only_chart(l3_data_iv_only, date_range):
    ...
    ax[0, 2].scatter(
        x=l3_data_iv_only["log_iv"],
        y=l3_data_iv_only["fitted_iv"],
        color="darkblue",
        alpha=0.1,
    )
    ax[0, 2].set_xlabel("log(IV)")
    ...
    ax[0, 2].plot(
        [min(l3_data_iv_only["log_iv"]), max(l3_data_iv_only["log_iv"])],
        [min(l3_data_iv_only["log_iv"]), max(l3_data_iv_only["log_iv"])],
        color="red",
        linestyle="--",
    )
    ...
    ax[1, 0].scatter(
        x=l3_data_iv_only.xs("C", level="cp_flag")["moneyness"],
        y=np.exp(l3_data_iv_only.xs("C", level="cp_flag")["log_iv"]),
        color="blue",
        alpha=0.1,
        label="IV",
    )
    ...
    ax[1, 1].scatter(
        x=l3_data_iv_only.xs("P", level="cp_flag")["moneyness"],
        y=np.exp(l3_data_iv_only.xs("P", level="cp_flag")["log_iv"]),
        color="red",
        alpha=0.1,
        label="IV",
    )
    ...
    ax[2, 0].scatter(
        x=l3_data_iv_only.xs("C", level="cp_flag")["moneyness"],
        y=l3_data_iv_only.xs("C", level="cp_flag")["rel_distance_iv"],
        color="blue",
        alpha=0.1,
        label="Calls",
    )
    ...
    ax[2, 1].scatter(
        x=l3_data_iv_only.xs("P", level="cp_flag")["moneyness"],
        y=l3_data_iv_only.xs("P", level="cp_flag")["rel_distance_iv"],
        color="red",
        alpha=0.1,
        label="Puts",
    )
    ...
    plt.suptitle(
        f"Level 3 Filtered Data: IV Filter Only: {date_range.replace('_', ' to ')}"
    )
    plt.tight_layout()
    # plt.show()
...
def build_l3_data_iv_pcp_chart(l3_filtered_options, date_range):
    ...
    chart_data = l3_filtered_options.reset_index().set_index(
        ["date", "exdate", "cp_flag"]
    )
    ...
    # Scatter plot with x=log_iv and y=fitted_iv
    ax[0, 2].scatter(
        x=chart_data["log_iv"], y=chart_data["fitted_iv"], color="darkblue", alpha=0.1
    )
    ax[0, 2].set_xlabel("log(IV)")
    ax[0, 2].set_ylabel("Fitted log(IV)")
    ax[0, 2].set_title("log(IV) vs Fitted log(IV)")
    # Add a 45-degree line
    ax[0, 2].plot(
        [min(chart_data["log_iv"]), max(chart_data["log_iv"])],
        [min(chart_data["log_iv"]), max(chart_data["log_iv"])],
        color="red",
        linestyle="--",
    )
    ...
    ax[1, 0].scatter(
        x=chart_data.xs("C", level="cp_flag")["moneyness"],
        y=np.exp(chart_data.xs("C", level="cp_flag")["log_iv"]),
        color="blue",
        alpha=0.1,
        label="IV",
    )
    ...
    ax[1, 1].scatter(
        x=chart_data.xs("P", level="cp_flag")["moneyness"],
        y=np.exp(chart_data.xs("P", level="cp_flag")["log_iv"]),
        color="red",
        alpha=0.1,
        label="IV",
    )
    ...
    ax[2, 0].scatter(
        x=chart_data.xs("C", level="cp_flag")["moneyness"],
        y=chart_data.xs("C", level="cp_flag")["rel_distance_iv"],
        color="blue",
        alpha=0.1,
        label="Calls",
    )
    ...
    ax[2, 1].scatter(
        x=chart_data.xs("P", level="cp_flag")["moneyness"],
        y=chart_data.xs("P", level="cp_flag")["rel_distance_iv"],
        color="red",
        alpha=0.1,
        label="Puts",
    )
    ...
    plt.suptitle(
        f"Level 3 Filtered Data: IV+Put-Call Parity Filters: {date_range.replace('_', ' to ')}"
    )
    plt.tight_layout()
    # plt.show()
