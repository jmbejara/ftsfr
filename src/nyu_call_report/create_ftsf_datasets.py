...
df = df_all[["rssdid", "date", "leverage"]].sort_values(by=["rssdid", "date"]).reset_index(drop=True)
df = df.rename(
    columns={"rssdid": "entity", "date": "date", "leverage": "value"}
)
...
df_bhc = df_all[["bhcid", "date", "assets", "equity", "cash"]].groupby(["bhcid", "date"]).sum()
df_bhc = df_bhc.reset_index()
df_bhc["leverage"] = df_bhc["assets"] / df_bhc["equity"]

df = df_bhc[["bhcid", "date", "leverage"]].sort_values(by=["bhcid", "date"]).reset_index(drop=True)
df = df.rename(
    columns={"bhcid": "entity", "date": "date", "leverage": "value"}
)
...
df = df_all[["rssdid", "date", "cash_liquidity"]].sort_values(by=["rssdid", "date"]).reset_index(drop=True)
df = df.rename(
    columns={"rssdid": "entity", "date": "date", "cash_liquidity": "value"}
)
df.to_parquet(DATA_DIR / "ftsfa_nyu_call_report_cash_liquidity.parquet")

## nyu_call_report_holding_company_cash_liquidity
df = df_all[["bhcid", "date", "cash_liquidity"]].sort_values(by=["bhcid", "date"]).reset_index(drop=True)
df = df.rename(
    columns={"bhcid": "entity", "date": "date", "cash_liquidity": "value"}
)
