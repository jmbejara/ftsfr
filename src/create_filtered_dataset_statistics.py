...
def apply_forecasting_preprocessing(df, frequency, seasonality, test_split=0.2):
    ...
    return train_data_before, train_data_filtered, full_data_filtered, original_count, final_count
...
def calculate_filtering_statistics(dataset_info):
    ...
    try:
        ...
        try:
            train_before, train_after, full_after, original_count, final_count = apply_forecasting_preprocessing(
                df, dataset_info['frequency'], dataset_info['seasonality']
            )
            
            # Calculate statistics BEFORE filtering
            entities_before = train_before["unique_id"].n_unique()
            lengths_before = (train_before
                             .group_by("unique_id")
            ...
            return {
                ...
                'median_length_after': int(median_length_after) if median_length_after else 0,
            ...
            if "No series meet the minimum length requirement" in str(ve):
                ...
                drop_cols = [c for c in df_processed.columns if c.startswith("__index_level_")]
                if drop_cols:
                ...
                return {
                    ...
                    'entities_before': int(entities_before),
                    'entities_after': 0,
                    'median_length_before': int(median_length_before) if median_length_before else 0,
                    'median_length_after': 0,
...
def create_csv_table(stats_list, output_path):
    ...
    # Reorder columns
    column_order = [
        'table_name', 'group', 'frequency', 'entities_before', 'entities_after', 
        'median_length_before', 'median_length_after', 'min_date', 'max_date'
    ]
...
def main():
    ...
    for dataset_info in active_datasets:
        ...
        if stats.get('error'):
            ...
            if stats['entities_after'] == 0:
                print(f"    Entities: {stats['entities_before']} → 0 (ALL FILTERED OUT - series too short)")
                print(f"    Median length before: {stats['median_length_before']} (insufficient for forecasting)")
            else:
                print(f"    Entities: {stats['entities_before']} → {stats['entities_after']} ({entities_removed} removed)")
                print(f"    Median length: {stats['median_length_before']} → {stats['median_length_after']}")
    ...
    total_entities_before = sum(s['entities_before'] for s in all_stats if isinstance(s['entities_before'], int))
    total_entities_after = sum(s['entities_after'] for s in all_stats if isinstance(s['entities_after'], int))
