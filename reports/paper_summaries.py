...
def find_matching_md_file(bib_key: str, bib_entry: Dict, references_dir: Path, client: OpenAI) -> Path:
...
def main():
    ...
    for i, (bib_key, bib_entry) in enumerate(new_entries.items(), 1):
        ...
        if md_file:
            ...
            author, year = extract_author_year(bib_entry)
            unmatched_entries.append((bib_key, bib_entry, f"Could not find file for {author} ({year})"))
