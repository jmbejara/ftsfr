...
\citet{Dickerson2024} document that the bid--ask-averaged TRACE prices used to construct the credit-spread signal contain market-microstructure noise (MMN) that mechanically distorts cross-sectional sorts. The Open Source Bond Asset Pricing panel exposes the relevant variation directly: each bond-month carries both the MMN-biased credit spread \texttt{cs} (the conventional construction) and the MMN-corrected credit spread \texttt{CS} (the construction recommended by \citealp{Dickerson2024}). The realized monthly return is held fixed across the two conventions, so any downstream difference comes from cleaning the signal alone.

Before turning to the forecasting comparison, we first replicate the underlying \citet{Dickerson2024} result in the data: holding the credit-spread sort fixed, switch only the realised-return convention between \texttt{ret\_vw} (the standard month-end TRACE return, whose start-of-period price is the same bid--ask-averaged quote that defined the signal at the end of the prior month, so any residual microstructure noise enters both) and \texttt{ret\_vw\_bgn} (the month-begin convention recommended by the OSBAP project for implementable-return tests, which separates the signal date from the return-start date by a full bid--ask cycle and so breaks the mechanical signal--return link). Table~\ref{tab:mmn_dickerson} reports the resulting decile-10-minus-decile-1 long--short under each convention. The apparent credit-spread premium falls from 5.66\% per year under \texttt{ret\_vw} (with a $t$-statistic of 1.48) to 4.18\% per year under \texttt{ret\_vw\_bgn} (with a $t$-statistic of 1.15), a 26\% drop and a step from marginally significant to insignificant at conventional levels. The remaining 4.18\% is the part of the apparent premium that survives once the mechanical signal--return contamination is broken; the 1.48 percentage points that vanish are the microstructure-noise component that the Dickerson critique identifies. In their original construction, which uses contaminated signals as well as contaminated returns, the drop is larger (50--65\%); we recover roughly half of that magnitude using only the return-side fix that the OSBAP main panel directly supports.
...
\caption*{\scriptsize
\textbf{Note:} Each row reports the value-weighted long--short return (decile 10 minus decile 1) from sorting corporate bonds into ten value-weighted credit-spread deciles each month using the OSBAP \texttt{cs} signal, then computing the realised return under the indicated convention. Both rows use identical bond universes, sort dates, decile assignments, and weighting variables. The only difference is the realised-return column. \texttt{ret\_vw} uses month-end TRACE prices for both the signal at $t$ and the return-start at $t$, so any residual market-microstructure noise enters both; \texttt{ret\_vw\_bgn} uses month-begin TRACE prices for the return-start at $t+1$, separating signal-date and return-start prices by a full bid--ask cycle. $t$-statistics are Newey--West uncorrected (i.i.d.\ assumption) and are reported only for relative comparison. Sample: 269 months, 2002--2024.
\emph{Sources: openbondassetpricing.com (OSBAP 2025 main panel), Authors' analysis.}}
\end{table}

The 26\% drop on OSBAP credit spread is modest because the OSBAP main panel is already a strongly MMN-cleaned product---the bid--ask noise component that drives the mechanical CEIV bias has been largely removed from \texttt{cs} before any downstream user sees it. To exhibit the bias on data closer to the raw-TRACE convention that the literature has historically used, and on the signal where \citet{Dickerson2024} report the most extreme bias, we run the same Approach-1-versus-Approach-3 comparison on \emph{short-term reversal} (past-month return) using the clean\_trace pipeline (a restructured fork of the OSBAP/DRR TRACE-cleaning code that applies Dick-Nielsen 2014, Binsbergen-Nozawa-Schwert 2025, a decimal-shift corrector, and a bounce-back filter, but does not pre-smooth signal prices via multi-day averaging). Short-term reversal is the signal where the CEIV mechanism is most acute because the signal \emph{is} the recent return: any month-end price noise $\delta_{i,t}$ enters the signal mechanically and re-enters the return denominator one period later. We implement \citet{Dickerson2024}'s Eqns~(1) and (2) directly, including the coupon cash flow $C_{t+1}$ in the return numerator (recovered from jumps in the cumulative-coupon column \texttt{accpmt} in the stage1 panel) so that our return measurement matches theirs as closely as possible on the same data infrastructure. The only deliberate divergence is that we do not apply their special trading-in-default return formulas: we drop bond-months without consecutive trading observations, while they continue to track defaulted bonds on a flat-basis convention. Their full-procedure headline for this signal is a premium dropping from $-0.99\%$ per month ($t = -4.46$) to $-0.09\%$ per month ($t = -0.51$), with bias comprising over 90\% of the documented effect. Table~\ref{tab:str_dickerson} reports our replication.
...
\emph{Sources: case\_study\_clean\_trace stage1 panel (a restructured fork of the OSBAP/DRR TRACE-cleaning pipeline), Authors' analysis.}}
\end{table}

Together, Tables~\ref{tab:mmn_dickerson} and~\ref{tab:str_dickerson} bracket the empirical magnitude of LIB on our data. On the heavily-pre-cleaned OSBAP credit-spread signal the bias is modest (26\% relative reduction, marginal-to-insignificant transition) because most of the bid--ask noise has already been averaged out; on the lighter-pre-cleaned clean\_trace short-term-reversal signal the bias is dramatic (84\% relative reduction, strongly-significant-to-insignificant transition), matching \citet{Dickerson2024}'s headline finding to within rounding. The short-term-reversal exhibit is the strict replication: same pipeline as theirs (clean\_trace is a restructured fork of their TRACE-data-pipeline), same universe filters (Binsbergen-Nozawa-Schwert 2025), same return formulas with $C_{t+1}$ coupon cash flow recovered from cumulative-coupon tracking in the daily panel, and same overlapping bond sample. We turn now to the forecasting question.

On the same OSBAP panel we then build two parallel panels of value-weighted decile portfolios sorted on each cleaning convention of the signal itself, holding the universe and the realised-return series identical. The cross-decile rank correlation between the two signals is 0.98 and the resulting portfolio return time series are 98--99\% correlated within decile, yet the long--short spread (decile 10 minus decile 1) differs by roughly a factor of two (2.8\% per year under \texttt{cs} versus 5.6\% per year under \texttt{CS}). The implication is that cleaning the signal does not merely reproduce the same portfolio with cosmetic differences: it changes which bonds occupy each decile in a way that reshuffles the cross-section enough to alter both the apparent premium and, as we show next, the relative ranking of forecasting models. On these two near-identical-but-not-quite panels we run the full twelve-model suite used elsewhere in this paper, under the same rolling-origin cross-validation protocol. Table~\ref{tab:mmn_sensitivity} reports the result.
...
output frequency.  \texttt{wrdsapps\_bondret.bondret} is a monthly
end-of-month panel, with one observation per bond per month indexed on
\texttt{price\_eom}, so our basis series is constructed at month-end and
...
daily Markit bond pricing pull alongside the WRDS Bond Returns filter; the
merge, NSS curve fitting, and Z-spread machinery in our pipeline would
...
discard individual Z-spread fits that hit the brentq solver's
