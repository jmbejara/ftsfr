...
\begin{abstract}
    Accurate time-series forecasts underpin asset pricing, risk management, monetary and macroprudential policy, and supervisory stress testing. Yet the literature's verdict on \emph{which} forecasting methods perform best is unstable across studies: the M3, M4, and M5 competitions and the Monash Time Series Forecasting Archive deliver materially different rankings of statistical, machine-learning, and deep-learning methods \citep{Makridakis2018, Makridakis2020, Makridakis2022, Godahewa2021}. The standard explanations---genuine series-by-series heterogeneity and the No Free Lunch theorem \citep{Wolpert1997}---treat the data as a fixed substrate. We argue that an under-appreciated source of disagreement is that the substrate is \emph{not} fixed: two studies nominally evaluating the same financial series often use different data sources, sample windows, and cleaning conventions, so apparent method rankings entangle method skill with cleaning choices. We address this by introducing FTSFR, a standardized, open-source, license-respecting financial time series forecasting repository covering equities, corporate bonds, U.S.~Treasuries, foreign exchange, commodities, credit default swaps, and options, together with five basis-spread datasets and supervisory and intermediary indicators, each cleaned per the canonical paper for that asset class. FTSFR is the financial-domain analogue of FRED-MD \citep{McCracken2016} for macro and Monash \citep{Godahewa2021} for general time series, with full pipeline reproducibility (download, clean, format) given WRDS and Bloomberg subscriptions. Holding the data fixed and evaluating roughly a dozen methods under a single preprocessing pipeline and cross-validation protocol, we find that asset returns remain near-unforecastable ($R^2_{\text{oos}} \approx 0$) while basis spreads and supervisory indicators yield economically meaningful gains for global machine-learning methods---illustrating that ``which model is best'' is a task-specific question that requires a fixed-data benchmark to answer credibly.
    \end{abstract}
...
This paper introduces FTSFR, an open-source benchmark for financial time series forecasting. By holding the data substrate constant across methods---through reproducible, literature-faithful cleaning of every series---FTSFR isolates method skill from cleaning choices and turns ``which forecaster wins?'' into a question that can be answered reproducibly.
...
Second, the benchmark provides comprehensive coverage of financial markets with both aggregated portfolio and disaggregated security-level data. Coverage spans seven major asset classes, five basis-spread datasets following \citet{Siriwardane2021}, and supervisory and intermediary indicators including bank Call Reports \citep{Drechsler2017} and the He--Kelly--Manela intermediary capital factors \citep{He2017}.

Third, we establish reproducible baselines using twelve forecasting models---from classical statistical methods (ARIMA, ETS, Theta) to modern deep learning architectures (N-BEATS, N-HiTS, transformer variants, KAN)---all evaluated under one preprocessing pipeline and one rolling-origin cross-validation protocol. These baselines turn the benchmark into something researchers can build against rather than alongside.

Consistent with the M-competitions' lesson that no single method dominates universally, the empirical results underscore that model choice must respect the data-generating process---and they demonstrate that apparent disagreements across prior cross-study comparisons can be partly resolved once the data substrate is fixed. For asset returns, even the most sophisticated auto deep-learning architectures rarely beat the historical average; out-of-sample $R^2$ readings remain near zero across equities, corporate bonds, U.S.~Treasuries, FX, options, and CDS, reaffirming how thin the exploitable signal is and consistent with decades of empirical asset-pricing evidence. Basis spreads and supervisory indicators tell a different story: NBEATS, NHITS, and NLinear substantially improve MASE relative to historical averages and classical smoothing on basis-spread panels, while Theta and NHITS capture persistent dynamics in bank liquidity and leverage ratios. The asset-pricing, risk-management, and financial-stability use cases for these forecasts therefore call for different default models, with the per-task rankings reported in Section~\ref{sec:results} guiding the choice. We hope FTSFR provides the fixed substrate against which future methodological progress in financial time series forecasting can be measured.
