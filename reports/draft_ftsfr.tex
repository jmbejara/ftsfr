...
\paragraph{Technical Implementation.} The pipeline first normalises timestamps (e.g., aligns month-end series to true month-ends) and builds a canonical grid using per-entity gap filling. We then split train/test windows per series according to frequency-driven horizons and evaluate coverage requirements: monthly data must supply at least 63 observations in total (daily data 135), maintain sufficient variance, and stay below pre-set gap ratios. We record both the original entity counts and the retained subset, add gap indicators, and perform light forward-only imputation on the training window to support baselines; any series failing post-imputation validation is removed. The remaining train/test slices form the single source of truth for every model family.

\paragraph{Impact and Necessity.} Portfolio-level aggregates are mostly unaffected. For example, Treasury portfolios, CIP spreads, and the yield curve retain 100\% of entities while a few datasets such as the CDS portfolio panel fall to 20\% retention when the minimum-horizon rule is applied. Disaggregated panels experience the sharpest pruning: individual CDS contracts shrink from 6,552 to 78 series (1.2\% retention), corporate bonds from 23,473 to 6,347 (27\% retention), and Treasury bond securities from 2,054 to 543 (26.4\% retention). Regulatory filings sit in between. BHC cash-liquidity series drop from 13,770 to 5,103 entities (37.1\%), BHC leverage falls from 13,761 to 5,397 (39.2\%), while the broader bank-level leverage panel retains 70.3\% of series. These filters strike a balance—protecting neural models from degenerate inputs, preventing classical models from exploiting overly short histories, and keeping comparisons focused on genuine forecasting skill rather than artefacts of uneven preprocessing.
...
To provide an overall assessment of model performance across all datasets, Table~\ref{tab:median_mase_summary} presents summary statistics for each model across three key performance metrics. The table shows median and mean values for MASE (absolute performance), Relative MASE (performance relative to Historic Average baseline), and out-of-sample R² (predictive power), with models sorted by their median MASE performance. This comprehensive summary allows identification of models that consistently perform well across the diverse range of financial time series in our benchmark, providing guidance for practitioners seeking robust forecasting approaches.
...
% \input{../_output/forecasting/paper/smape_pivot_tabular.tex}
...
% \input{../_output/forecasting/paper/mae_pivot_tabular.tex}
