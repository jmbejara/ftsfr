...
\begin{abstract}
This paper presents an open-source macro-finance data repository for
...
\subsection{The need for standardized benchmarks}
\label{sec:need_for_standardized_benchmarks}
...
there is no single forecasting method that performs best for all time series and, as such, to improve forecasting in the financial domain, we need to evaluate forecasting methods on a suite of datasets from the financial domain. Furthermore, it's also important that these datasets reflect the datasets as they
commonly appear in the relevant finance literature. That is, they should use the same data sources and the same
domain-specific cleaning and normalization procedures established in the literature. 


Our proposed benchmark addresses this limitation by creating a standardized, literature-compliant repository for the macro-finance domain. By providing open-source scripts that automate the download and cleaning of data from sources like WRDS and Bloomberg, we assemble datasets spanning equities, bonds, FX, commodities, banking, and more. Each dataset is processed according to canonical methods from seminal academic papers, and crucially, while these foundational studies often lack publicly available replication code, our implementations reproduce their key results. This approach not only validates our data processing but also establishes a reusable framework that complements general time series benchmarks (like Monash, Libra, and TFB) by focusing specifically on financial data. The result is a resource that enables the kind of apples-to-apples model comparisons advocated by recent surveys, filling a critical hole in quantitative finance research.

Our repository provide a variety of aggregated and disaggregated datasets from a variety of financial asset classes. For example, data for options and credit default swaps (CDS) are provided on a contract-specific basis, while banking data from Call Reports are disaggregated at the bank-specific level.
This granularity enables nuanced research into micro-level market dynamics. Moreover, we present datasets from multiple analytical angles to capture their diverse economic roles. For example, in addition to provided datasets that measure the financial returns across various assets across a variety of asset classes, we also calculate the spreads associated with basis trades that uses these assets. 
The CDS data, for instance, is not only formulated as a return series on the CDS contracts but is also merged with bond data to construct the CDS-bond basis, a key arbitrage spread, at the contract level. 

This multi-faceted approach empowers researchers to investigate a richer set of phenomena, from price forecasting to the analysis of complex inter-market relationships.
...
In time-series analysis, a classic data pitfall is using non-stationary (e.g. trending) data in regression without proper detrending or differencing. Granger and Newbold \cite{GrangerNewbold1974} famously warned that regressing one growing time series on another can yield an apparently excellent fit (high R², significant t-statistics) that is actually spurious
jumbong.github.io
...
Duffie (1999) (THIS IS ANOTHER PAPER WE MAY NEED TO CITE) shows that cash froms from purchasing CDS protection on 
firm $i$'s bonds are the same as being long the risk-free floating rate note
and short the floating rate debt of firm $i$. Thus, the CDS spread should be equal to the 
floating rate spread for firm $i$. The floating rate spread of firm $i$ is defined as the spread of par floating rate debt
subtracting the riskless floating rate. Intuitively, an investor that buys both the corporate bond
and CDS protection faces no credit risk because the default will be paid off. 
...
\subsubsection{Z-spread construction}
