...
% Center every caption (title, Note, and Source) uniformly across all figures and tables
\captionsetup{justification=centering}
% Uniform "Source" note rendered below every figure and table (consistent size and styling)
\newcommand{\floatsource}[1]{\caption*{\footnotesize\emph{#1}}}

% Canonical source list for the aggregate, benchmark-wide tables and figures.
% Defined once so every benchmark-wide float stays in sync; edit here to update all.
\newcommand{\allsources}{Sources: Bloomberg, Board of Governors of the Federal Reserve System, Center for Research in Security Prices, Compustat, Call Reports, FINRA TRACE, OptionMetrics, S\&P Global, Open Source Bond Asset Pricing, He, Kelly, and Manela (2017), Authors' analysis}
...
\begin{abstract}
    Accurate time-series forecasts underpin asset pricing, risk management, monetary and macroprudential policy, supervisory stress testing, and other applications. The set of available forecasting methods is expanding rapidly, driven especially by new machine-learning models, which raises a practical question: do these methods deliver real forecasting-power gains on financial data? By the No Free Lunch theorem, this can only be settled empirically, on domain-specific data, and a fair comparison requires holding the data fixed across methods. Yet each new method is typically evaluated on its own dataset, built with its own sources, sample windows, and cleaning conventions, so apparent method rankings entangle method skill with cleaning choices. Cleaning financial data, moreover, requires domain expertise (which instruments are tradable, which contracts are faulty, whether liquidity suffices). We address this by assembling, in one place, canonical cleaning procedures for many financial datasets, so the data can be held fixed across forecasting experiments.
    We introduce a standardized open-source dataset covering equities, corporate bonds, U.S.~Treasuries, foreign exchange, commodities, credit default swaps, options, five basis-spread datasets, and bank and intermediary indicators, each cleaned per the canonical paper for that asset class.
...
Despite the stakes, and despite a rapidly expanding toolkit of forecasting methods driven especially by machine learning, no method has emerged as a universal winner, and the leaderboard shifts from one study to the next. The M3 competition concluded that classical statistical models dominate machine-learning alternatives both in accuracy and in computational cost \citep{Makridakis2018}. The M4 competition's winner was a hybrid recurrent-neural-network and exponential-smoothing system, with the strongest pure machine-learning methods placing in the middle of the field \citep{Makridakis2020}. Just two years later, the M5 competition was swept by pure machine-learning methods, every one of which beat the best statistical baselines and combinations thereof \citep{Makridakis2022}. The Monash Time Series Forecasting Archive likewise reports no uniform winner across its 25 datasets \citep{Godahewa2021}. These shifts are consistent with (i) genuine series-by-series heterogeneity in dynamics and (ii) the No Free Lunch theorem \citep{Wolpert1997}: no single algorithm is best for all data-generating processes. The implication is practical: to learn whether a new method genuinely improves financial forecasting, one must evaluate it on financial data, with the data held fixed across the methods being compared.
...
\bottomrule
\end{tabular}
\floatsource{Sources: Authors' analysis}
\end{table}
...
Corporate Bond & Monthly returns for individual corporate bonds from TRACE. & \cite{Dickerson2024} \\
...
FF25 Size-BM & Daily Fama-French 25 portfolios: size and book-to-market & \cite{Fama1993} \\
FX & Daily foreign exchange returns vs USD & \cite{Lettau2014} \\
SPX Options Portfolios & Monthly leverage-adjusted returns for SPX option portfolios (54 CJS + 18 HKM) & \cite{Constantinides2013} \\
Treasury Bond & Monthly returns for individual Treasury bonds from CRSP & \cite{Gurkaynak2007} \\
Treasury Portfolio & Monthly returns for Treasury bond portfolios by maturity & \cite{Gurkaynak2007} \\
\midrule
...
Bank Leverage & Quarterly leverage ratios from call report data & \cite{Drechsler2017} \\
BHC Cash Liquidity & Quarterly bank holding company cash liquidity & \cite{Drechsler2017} \\
BHC Leverage & Quarterly bank holding company leverage ratios & \cite{Drechsler2017} \\
HKM Daily Factor & Intermediary risk factors: the capital risk factor (AR(1) innovation in the capital ratio) and the value-weighted intermediary investment return & \cite{He2017} \\
HKM Monthly Factor & Same as above, but monthly & \cite{He2017} \\
...
\midrule
Bank Cash Liquidity & Call Reports, following \citet{Drechsler2017}$^{c}$ \\
Bank Leverage & Call Reports, following \citet{Drechsler2017} \\
BHC Cash Liquidity & Call Reports, following \citet{Drechsler2017} \\
BHC Leverage & Call Reports, following \citet{Drechsler2017} \\
...
\item[$^{d}$] See \url{https://asaf.manela.org/data/}
...
For each asset class, we provide both aggregated portfolio returns (matching the test portfolios used in HKM) and disaggregated security-level data. The disaggregated datasets apply identical filtering criteria, subsampling, and transformations as specified in the original papers, but preserve individual security information rather than aggregating into portfolios. 
...
Commodity Index (GSCI) with directly computed monthly returns for 23 commodities.
...
\begin{figure}[h]
  ...
  \end{tabular}
  \floatsource{Sources: Bloomberg, Authors' analysis}
...
noise introduced by the 2009 ``Big Bang'' contract change. Raw S\&P Global XR quotes
...
Covered interest parity (CIP), once ``the closest thing to a physical law in international finance,'' is a leading example: it has been systematically violated since 2008 \citep{Du2018}. The size of the violation is measured by the cross-currency basis, the gap between the cost of borrowing dollars directly and the cost of borrowing them synthetically by lending a foreign currency and hedging the exchange rate forward. When dollar funding is scarce, foreign institutions bid up the synthetic route and the basis widens, so the basis is effectively a price of dollar funding stress. It spikes sharply during crises: in 2008 and again in March 2020 the basis against major currencies widened dramatically as foreign institutions scrambled for dollars \citep{BoardofGovernorsoftheFederalReserveSystem2020}.
Because the basis is such a direct gauge of funding stress, central banks monitor it: the Federal Reserve's central-bank swap lines, which lend dollars to foreign central banks during market stress, are designed precisely to relieve the dollar shortage that drives these deviations.
...
\begin{figure}[h]
  ...
  \end{tabular}
  \floatsource{Sources: Center for Research in Security Prices, FINRA TRACE, S\&P Global, Authors' analysis}
  \label{fig:cds_basis}
\end{figure}

\paragraph{Covered Interest Parity}
...
\bottomrule
\end{tabular}
\caption*{\footnotesize \textbf{Note:} ``Seasonal'' indicates built-in mechanisms for seasonal components; ``Exogenous'' denotes native support for additional regressors; ``Prob.'' captures whether the implementation produces full predictive distributions rather than point forecasts. $^{\dagger}$N-BEATSx extends the architecture to exogenous inputs. Attributes are assessed from the original model publications and Nixtla documentation (\url{https://nixtlaverse.nixtla.io/}).}
\floatsource{Sources: Authors' analysis}
\end{table}


\subsection{An Illustration: What Do These Forecasts Look Like?}
\label{subsec:forecast_illustration}

Before turning to the formal evaluation, Figure~\ref{fig:example_forecasts} offers a purely illustrative look at how differently these model families behave when extrapolating two contrasting kinds of data. To avoid singling out any individual series, each panel shows a cross-sectional average: we forecast every entity in the panel and then average both the realized series and the per-method forecasts across entities. Panel~(a) is the Treasury spot-futures (``cash-futures'') basis of Figure~\ref{fig:treasury_sf_basis}, averaged across its five tenors and forecast over the twelve months of 2019 (trained through December 2018). Panel~(b) is bank holding company (BHC) cash liquidity, averaged across the BHCs that report continuously over the window and forecast over the twelve quarters of 2017--2019 (trained through 2016). Both panels show the same representative quartet---one model from each family in our roster: the classical statistical workhorses ARIMA and Theta, the strongest hybrid linear method on basis spreads (NLinear), and the strongest pure deep-learning method on basis spreads (N-BEATS), per the basis-spread panel of Table~\ref{tab:model_summary_by_category}. We hold the quartet fixed across panels for comparability and train the neural models globally over each panel's entities, consistent with our univariate-global design.

This exhibit is deliberately not an accuracy test. Whereas the benchmark evaluates each series one step ahead and rolls the origin forward (Section~\ref{sec:methodology}), here we generate a single twelve-step path so that the qualitative differences in shape are visible at a glance. The contrast across panels is the point. On the basis spread (Panel~a) the methods fan out: ARIMA and Theta anchor near recent levels and mean-revert, while the hybrid and deep-learning models trace more articulated paths that bend with the recent dynamics. On bank cash liquidity (Panel~b), by contrast, the four methods cluster much more tightly around the realized average, mirroring our finding that classical and machine-learning methods are roughly competitive on bank and intermediary indicators. None of this establishes which method forecasts best---that is the role of the error metrics in Section~\ref{sec:results}---but it conveys what ``a forecast'' concretely means for each family and why the appropriate default is task-specific.

\begin{figure}[htbp]
    \centering
    \caption{Illustrative Forecasts: Basis Spread vs.\ Bank Cash Liquidity}
  \begin{tabular}{@{}c@{}}
    \includegraphics[width=.95\linewidth]{\PathToOutput/forecasting/paper/example_forecasts_illustration.png}
  \end{tabular}
  \caption*{\footnotesize \textbf{Note:} Each panel plots a cross-sectional average: every entity is forecast individually, and the realized series and per-method forecasts are then averaged across entities. \emph{Panel (a):} Treasury spot-futures basis averaged across its five tenors (monthly), trained through December 2018 (dotted line) and forecast over 2019. \emph{Panel (b):} BHC cash liquidity averaged across banks reporting continuously over the window (quarterly), trained through 2016~Q4 and forecast over 2017--2019. One representative model is shown from each family: the classical statistical methods (ARIMA, Theta), the best hybrid linear method on basis spreads (NLinear), and the best pure deep-learning method on basis spreads (N-BEATS). The figure illustrates the qualitative shape of each family's forecast and is not an accuracy comparison; for that, see Section~\ref{sec:results}.}
  \floatsource{Sources: Bloomberg, Call Reports, Authors' analysis}
  \label{fig:example_forecasts}
\end{figure}


\subsection{Data Preprocessing}

To ensure fair model comparisons, our forecasting system applies a unified preprocessing pipeline before any model is trained. We summarize the rules below and report per-dataset retention statistics, median series lengths, and adaptive minimum observation thresholds in Table~\ref{tab:filtered_dataset_stats} of Appendix~\ref{app:forecasting_preprocessing}.

\paragraph{Filtering Methodology and Fairness Guarantees.} We drop any series too short to give a model a usable train/test split, and we hand the same filtered panel to every model. The length cutoff depends on the dataset: a series must have at least as many observations as the forecast horizon plus a six-observation buffer, and never fewer than 18 (seasonal series must clear a higher bar so that a full seasonal cycle remains). The one exception protects small datasets. If a dataset has ten or fewer entities, we skip filtering entirely and keep all of them, so that a whole category is not lost. Because the cutoff depends only on series length and not on the model, the statistical and neural estimators are always compared on identical data.

\paragraph{Technical Implementation.} The pipeline first normalizes timestamps (e.g., aligns month-end series to true month-ends) and builds a canonical date grid, filling gaps per entity. We then split each series into train and test windows using frequency-specific horizons: daily data are forecast roughly a calendar month ahead (30 calendar or 21 trading days), monthly data one month ahead, and quarterly data one quarter ahead. The horizon sets the coverage each series must meet: monthly panels need roughly 16 observations in total but only a single hold-out point, while daily and business-day panels must supply 21--30 observations in the test window. After splitting, we record the original and retained entity counts, add gap indicators, and perform light forward-only imputation on the training slice; any series that fails validation after imputation is discarded. The resulting train/test slices are the single source of truth for every model family.

\paragraph{Forecast Horizons and Cross-Validation.} All models, statistical and neural alike, are evaluated with rolling-origin cross-validation. The hold-out horizon matches the per-frequency target above, and the step size equals the horizon, so successive windows do not overlap. We use up to six windows, but the actual count is capped by the shortest surviving series in the dataset. For a monthly panel, this means forecasting one month ahead, then rolling the origin forward one month and forecasting again, for up to the last six month-ends; a daily panel instead forecasts the next 21 trading days, rolls forward 21 days, and repeats. A panel whose shortest series has room for only one such window therefore yields a single end-of-sample forecast, while longer panels yield six. The same window schedule is passed to both StatsForecast and NeuralForecast so baseline and neural estimates remain comparable, and the Auto wrappers reuse these folds during their internal hyperparameter searches. This design captures ``one-month-ahead'' predictability while avoiding evaluation windows so long that they exhaust shorter financial histories, and it provides repeated, out-of-sample checks that guard against overfitting.

\paragraph{Impact and Necessity.} How much a panel loses to the length filters depends on how aggregated it is. Portfolio- and index-level panels keep nearly all of their series: CIP spreads, TIPS/Treasury bases, and Treasury swap spreads retain 100\%, and the CDS bond basis panel keeps 2,608 of 2,776 entities (93.9\%). Monthly portfolio returns are similarly intact, the one exception being the CDS portfolio, which retains 4 of 20 entities (20\%). Disaggregated panels bear the brunt of the filters: individual CDS contracts shrink from 6,552 to 234 series (3.6\% retention), corporate bonds from 53,389 to 28,581 (53.5\%), and Treasury bond securities from 2,054 to 1,912 (93.1\%). Regulatory filings sit in between: BHC cash-liquidity series fall from 13,770 to 6,351 entities (46.1\%), BHC leverage from 13,761 to 6,653 (48.3\%), while bank-level leverage retains roughly three quarters (22,965 to 17,295; 75.3\%). These filters strike a balance, protecting neural models from degenerate inputs, preventing some models from exploiting overly short histories, and keeping comparisons focused on genuine forecasting skill rather than artifacts of uneven preprocessing.
...
\paragraph{Notation.} Let $\{y_{s,t}\}$ denote the target value for series $s$ at time $t$, $\{\hat y_{s,t}\}$ the corresponding forecast, $T_s$ the length of the test window for series $s$, and $\bar{y}_{\text{train},s}$ the training-sample mean of series $s$. When a metric is computed for a single series we suppress the $s$ subscript.
...
\medskip
\noindent\textbf{Out-of-Sample $R^2$ ($R^2_{\text{oos}}$).} For each (dataset, model) cell we report the pooled, panel-wide $R^2_{\text{oos}}$:
\[
R^2_{\text{oos}} = 1 - \frac{\sum_{s}\sum_{t=1}^{T_s} (y_{s,t}-\hat y_{s,t})^2}{\sum_{s}\sum_{t=1}^{T_s} (y_{s,t}-\bar{y}_{\text{train},s})^2}.
\]
Squared errors are summed across all series and all test-window observations in the dataset; the benchmark is each series' own training-sample mean (the Historic Average baseline). This is the standard panel definition in the asset-pricing forecasting literature \citep{Campbell2008,Gu2020}. Positive values indicate the model outperforms the historical-average benchmark on the dataset as a whole; zero or negative values indicate underperformance. We pool rather than averaging per-series $R^2$ values because, on disaggregated panels with heterogeneous variance (e.g.\ CRSP equity returns, individual TRACE bonds, CDS contracts), per-series ratios are sensitive to a handful of low-variance series whose benchmark $\sum_t (y_{s,t}-\bar{y}_{\text{train},s})^2$ is small, which can dominate a simple mean.

\section{Baseline Results and Analysis}
\label{sec:results}


We present forecasting results across all datasets and models, reporting three tables: Mean Absolute Scaled Error (MASE), Root Mean Square Error (RMSE), and out-of-sample $R^2$ ($R^2_{\text{oos}}$). MASE provides scale-free comparison across different time series, RMSE captures the magnitude of forecasting errors in original units, and $R^2_{\text{oos}}$ measures predictive power relative to the historical-average benchmark.
...
\caption*{\footnotesize \textbf{Note:} Values show Mean Absolute Scaled Error (MASE). Lower values indicate better performance. -- indicates missing results.}
\floatsource{\allsources}
\end{table}
...
\caption*{\footnotesize \textbf{Note:} Values show Root Mean Square Error (RMSE). Lower values indicate better performance. -- indicates missing results.}
\floatsource{\allsources}
\end{table}
...
\caption*{\footnotesize \textbf{Note:} Values show out-of-sample $R^2$ ($R^2_{\text{oos}}$). Positive values indicate better performance than the historical average baseline; negative values indicate underperformance. Higher values indicate better performance. Bold values highlight the best performing model for each dataset.}
\floatsource{\allsources}
\end{table}
...
\begin{figure}[htbp]
    ...
    \caption{Out-of-Sample $R^2$ ($R^2_{\text{oos}}$) Heatmap by Dataset and Model}
    \caption*{\footnotesize \textbf{Note:} Warmer colors indicate larger improvements over the Historic Average baseline, while cooler colors indicate underperformance. Cells with extreme values are annotated to preserve readability.}
    \floatsource{\allsources}
    \label{fig:r2oos_heatmap}
\end{figure}

To synthesize performance across datasets, Table~\ref{tab:median_mase_summary} reports median and mean statistics for MASE and out-of-sample $R^2$. Because MASE is the workhorse metric in the forecasting literature, we use it to anchor the discussion: \texttt{NBEATS} delivers the lowest median MASE (0.779), with \texttt{NHITS} (0.806), \texttt{TiDE} (0.806), and \texttt{DLinear} (0.790) within a few hundredths. All of these auto-deep models handily outperform classical baselines such as the Historic Average, the seasonal naïve benchmark, and simple exponential smoothing. Their automated hyperparameter search concentrates capacity on recurring seasonal windows, which is particularly valuable for the high-frequency panels in our benchmark. When we pivot to $R^2_{\text{oos}}$, however, the magnitudes shrink dramatically: \emph{every} model in the suite, including the classical baselines, posts a median $R^2_{\text{oos}}$ within $\pm 0.08$ of zero (and all but \texttt{Theta}, at $0.072$, within $\pm 0.03$). \texttt{ARIMA} produces the strongest mean (0.159), with \texttt{TiDE} (0.098) the only other model in solidly positive territory; among the rest, \texttt{Theta} and \texttt{KAN} sit near $-0.13$ to $-0.14$ mean while \texttt{NBEATS} ($-0.486$) is the clear laggard. The takeaway from the pivot is that an aggregate $R^2_{\text{oos}}$ near zero is the rule rather than the exception in this univariate benchmark: the historical-average benchmark itself is already the right forecaster for the typical panel, and gains on MASE come from sharpening the level forecast rather than from extracting time-series predictability that would translate into a positive $R^2_{\text{oos}}$.

\begin{table}[htbp]
\centering
\caption{Overall Model Performance Summary: MASE and R² Across All Datasets}
\label{tab:median_mase_summary}
\footnotesize
% \resizebox{\textwidth}{!}{%
\input{\PathToOutput/forecasting/paper/median_mase_summary_tabular.tex}
% }
\caption*{\footnotesize \textbf{Note:} Summary statistics across all datasets for each model. MASE shows absolute performance (lower is better), and R² shows out-of-sample predictive power (higher is better). Models are sorted by median MASE.}
\floatsource{\allsources}
\end{table}
...
\caption*{\footnotesize \textbf{Note:} Metrics are computed within each dataset category (Basis Spreads, Returns, Other). Lower MASE values indicate better performance; higher $R^2_{\text{oos}}$ values indicate better performance.}
\floatsource{\allsources}
\end{table}
...
Basis spreads tell a different story. On median MASE the multi-resolution \texttt{NBEATS} variant leads (0.407), with the classical \texttt{Theta} (0.411), \texttt{NLinear} (0.425), and \texttt{KAN} (0.430) methods clustered closely behind. These deep architectures excel because they decompose the series into trend and seasonal components while borrowing strength across entities through global training. Basis spreads exhibit pronounced seasonal structure (e.g., quarter-end funding pressure) and medium-term mean reversion, and the auto wrappers concentrate the search on those horizons. The clear separation appears on $R^2_{\text{oos}}$: the deep models (NHITS, NLinear, NBEATS, TiDE) all post median $R^2_{\text{oos}}$ above 0.64, well clear of the best classical contender (\texttt{ARIMA} at 0.53), so they deliver large improvements over the Historic Average and simple exponential smoothing baselines and widen the gap over classical methods on these spreads.

The ``Other" category, which includes bank regulatory filings, volatility indicators, and macro-financial composites, is more nuanced. \texttt{Theta} delivers the lowest median MASE (0.811) by a hair over \texttt{NHITS} (0.823), \texttt{NLinear} (0.820), and \texttt{DLinear} (0.821), and \texttt{NHITS} edges out \texttt{Theta} on median $R^2_{\text{oos}}$ (0.458 vs.\ 0.455), with \texttt{ARIMA} just behind (0.442). These three deliver materially stronger gains over the Historic Average than any of the heavier deep models or DeepAR on this segment. These datasets are typically longer and smoother than the high-volatility returns series but lack the strong seasonal spikes of basis spreads, so the flexible trend extrapolation in Theta, the multi-resolution decomposition in NHITS, and the parsimonious ARIMA dynamics all fit naturally. The auto cross-validation helps these simpler structures stay calibrated without overfitting idiosyncratic shocks.

Taken together, the cross-metric evidence confirms that performance is conditional: MASE-based rankings highlight the strength of NBEATS/NHITS/TiDE/DLinear on error scales commonly used by practitioners, while out-of-sample $R^2$ separates a small set of methods (\texttt{ARIMA}, \texttt{TiDE}) that beat the historical mean on average from a much larger set that does not. Analysts selecting a forecast should therefore align the evaluation metric with the economic decision. $R^2_{\text{oos}}$ is appropriate for return forecasting, where the historical mean is a credible trading benchmark. Scaled absolute errors are appropriate for arbitrage spreads, where seasonal accuracy matters more than mean-squared scorekeeping, and for bank and intermediary indicators, where parsimonious level forecasts capture most of the persistent dynamics.

Tables~\ref{tab:median_mase_summary}--\ref{tab:model_summary_by_category} rank individual models, but a practitioner often asks a coarser question: as a class, do deep-learning architectures actually beat classical statistical methods, or do the recent hybrid decomposition-based models bridge the gap? Table~\ref{tab:model_summary_by_type} aggregates the same metrics into four groups of models, using the taxonomy from Table~\ref{tab:model_properties} and separating out the Historic Average baseline because it is the benchmark against which $R^2_{\text{oos}}$ is defined: \textit{benchmark} (Historic Average), \textit{classical statistical} (ARIMA, SES, Theta), \textit{hybrid} (DLinear, NLinear), and \textit{deep learning} (DeepAR, N-BEATS, N-HiTS, Vanilla Transformer, TiDE, KAN). Panel~A pools across all datasets; Panel~B repeats the comparison within each dataset category.
...
\caption*{\footnotesize \textbf{Note:} Each row aggregates every (model $\times$ dataset) observation whose model belongs to the indicated method type. ``\# Models'' counts distinct models in the type and ``N'' counts (model $\times$ dataset) observations. The Historic Average benchmark is reported in its own row because it is the benchmark against which $R^2_{\text{oos}}$ is defined. Lower MASE indicates better performance; higher $R^2_{\text{oos}}$ indicates better performance. Best values within each panel (or, in Panel~B, within each dataset category) are bolded.}
\floatsource{\allsources}
\end{table}

The aggregated view sharpens the qualitative picture from the model-level tables. On the MASE scale the three method families finish much closer than the individual rankings might suggest: hybrid wins median MASE (0.805), with deep learning and classical within a few hundredths (0.839 and 0.850 respectively). The out-of-sample $R^2$ column now tells a much milder story than the per-model variation suggested. All three families post a \emph{median} $R^2_{\text{oos}}$ within $0.01$ of zero (classical 0.010, hybrid 0.002, deep learning 0.006), and the \emph{mean} $R^2_{\text{oos}}$ is mildly negative for each family (classical $-0.034$, hybrid $-0.043$, deep learning $-0.110$). No family posts a solidly positive aggregate mean. The mild gap in the means reflects a small number of disaggregated panels on which deep learning underperforms the historical-average benchmark; the median---which is robust to those tail cases---is essentially zero for every family. Panel~B shows that the cross-family gap is concentrated in the basis-spreads category, where hybrid and deep-learning models exploit the seasonal structure of funding-related spreads and beat the classical aggregate on median MASE (0.478 and 0.541 vs.\ 0.691) and on $R^2_{\text{oos}}$ (median 0.647 and 0.643 vs.\ 0.345). On the Returns category every family clusters within a few hundredths of MASE and slightly negative $R^2_{\text{oos}}$. The Other category (bank regulatory filings and intermediary indicators) is the most nuanced: median $R^2_{\text{oos}}$ now favors deep learning (0.430) and classical statistical methods (0.393) almost equally, with hybrid behind (0.378), while mean $R^2_{\text{oos}}$ inverts the median ranking---classical (0.340) edges hybrid (0.270) and deep learning (0.073) because the classical baselines avoid the tail failures that drag the deep-learning mean. The takeaway is not that one family dominates uniformly. Rather, the value-add of moving from a classical method to a deep or hybrid architecture is concentrated in segments with persistent seasonal or cross-sectional structure (basis spreads), is essentially absent in segments where the historical mean is already an oracle (returns), and is more variable in segments where parsimony pays off (bank and intermediary indicators).

\subsection{Analysis by Model Type}

Traditional statistical models like Theta, SES, and ARIMA remain competitive across many datasets, particularly for univariate series with clear temporal patterns. Among deep learning models, the results are mixed: Transformer-based models demonstrate strong performance on disaggregated datasets but struggle when data is limited, NBEATS and NHITS perform well on certain datasets but do not consistently outperform simpler alternatives, and the newer KAN architecture appears in only a subset of datasets and is not consistently ranked. Notably, linear deep models like DLinear and NLinear achieve competitive results on many datasets despite their simplicity.

DeepAR exhibits relatively weak out-of-sample results across several datasets relative to the other deep learning architectures (Table~\ref{tab:median_mase_summary}). All neural models in this benchmark use Nixtla's Auto implementations,\footnote{\url{https://nixtlaverse.nixtla.io/}} which automate hyperparameter selection through Bayesian optimization and related search strategies. The Auto wrappers (AutoDeepAR, AutoNBEATS, AutoNHITS, and others) search over model-specific hyperparameters (e.g., hidden size, dropout, learning rate for DeepAR; stack types and block depth for NBEATS) using cross-validation on the training data. These searches do not guarantee optimal performance for every individual dataset.

The central objective of this paper is to provide a comprehensive benchmark of financial datasets that can be used by both practitioners and time-series forecasting researchers. We establish baseline results using off-the-shelf Auto methods from Nixtla to demonstrate feasible performance levels and to highlight which model families show promise on different types of financial data. Achieving the best forecast for any given dataset may require domain-specific expertise and manual tuning beyond a generic hyperparameter search. The benchmark's value lies not in claiming to provide optimal forecasts for every model, but in offering standardized datasets and reproducible pipelines that enable researchers to systematically investigate such questions. Future work can leverage these datasets to conduct focused hyperparameter sensitivity analyses, explore ensemble methods, or incorporate domain knowledge, which fall outside the scope of establishing the benchmark itself.
...
Before turning to forecasting, we confirm that the MMN correction is a meaningful, literature-endorsed cleaning choice rather than an arbitrary perturbation. We hold the credit-spread sort fixed and switch only the realised-return convention, from the standard month-end measure ($R^{\text{end}}$, whose start-of-period price reuses the same bid--ask-averaged quote that defined the signal, so microstructure noise enters both) to the OSBAP month-begin measure ($R^{\text{bgn}}$, which separates the signal and return-start prices by a full bid--ask cycle and breaks the mechanical link). Table~\ref{tab:mmn_dickerson} shows the apparent credit-spread premium falling from 5.66\% per year ($t = 1.48$) to 4.18\% per year ($t = 1.15$), a 26\% drop that moves it from marginally significant to insignificant. The reduction is modest here only because the OSBAP main panel is already heavily MMN-cleaned; on rawer signals the same bias is far larger, with \citet{Dickerson2024} documenting reductions exceeding 90\% on short-term reversal.
...
The contrasts span a range of cleaning intensity, which we summarize by how correlated the two variants' within-bucket return series remain. The SPX options contrast is the widest (average within-bucket correlation 0.68): we apply the three-level filter stack of \citet{Constantinides2013} either in full or only at Level 1, so the Level-1-only panel retains the extreme-implied-volatility, off-grid-moneyness, and put-call-parity-violating quotes that the canonical recipe drops. The Fama-French 25 contrast sits in the middle (correlation about 0.90): we switch only the size and book-to-market breakpoints between NYSE-only (the canonical convention) and all-CRSP, which loads the small-cap corners with microcaps. The Treasury contrast is the closest to the MMN setup (correlation above 0.99): we switch only the universe filter between a permissive set and the \citet{Gurkaynak2007} strict filters, which drop callables and recent issues. Table~\ref{tab:cleaning_robustness} stacks the three panels under the same column layout used in Table~\ref{tab:mmn_sensitivity}.
...
\floatsource{Sources: Center for Research in Security Prices, Compustat, OptionMetrics, Authors' analysis}
\end{table}

The Options panel (Panel~A) shows the same shape of lesson as the MMN case study: a reshuffled leaderboard rather than a uniform shift. No model clears $R^2_{\text{oos}} = +0.27$ on either variant, but within that band the ranking reorders sharply and in mixed directions, with three neural architectures (NBEATS, NHITS, KAN) gaining from the heavier L3 cleaning while five deep models and three classical baselines lose. The leaderboard top moves from DLinear/DeepAR on L1 to NHITS on L3, every one of the eight deep architectures shifts by $|\Delta R^2_{\text{oos}}| > 0.11$, and five cross zero, while the four classical baselines move by $0.15$ or less. The heavier L3 cleaning is also the defensible convention rather than a stylistic preference: its filters drop deep out-of-the-money and far-from-maturity options with bid--ask spreads exceeding 50\% of price and near-zero volume, quotes no market maker would fill at scale, so the L1 panel admits returns no investor could realise even in principle.

The FF25 panel (Panel~B) sits in the middle. Almost every $R^2_{\text{oos}}$ is negative (NLinear is the only model positive on either variant, and only barely), consistent with the broader equity-return result in Section~\ref{sec:results}. No model dominates either variant, so there is no leaderboard flip to report, but the deep models still swing materially (Transformer by $-0.441$, the largest in the panel, with NBEATS, DeepAR, KAN, and TiDE all moving by more than $0.13$) while the classical baselines are essentially unchanged ($|\Delta R^2_{\text{oos}}| \leq 0.04$). The magnitudes are large enough that a paper claiming a particular deep model ``works on FF25'' could see its claim reversed by switching breakpoint conventions.

The Treasury panel (Panel~C) is the closest analogue to the MMN setting: the two universes are 99\%+ correlated within bucket, yet the leaderboard reorders sharply. The classical baselines dominate the top of the ranking under both filter choices---all three of the top spots under the strict filter, and two of the top three (ARIMA and Theta, with TiDE breaking in at second) under the permissive filter---and barely move ($|\Delta R^2_{\text{oos}}| \leq 0.02$), while the deep models swing: TiDE falls from near the top on permissive ($0.272$) to negative on strict ($\Delta = -0.315$), NBEATS makes the largest fall in the panel ($\Delta = -0.375$), and NHITS and Transformer recover materially in the other direction. As in the TRACE case, the entire ranking volatility lives among the deep models, with classical baselines essentially stable.

Two qualifications carry over. First, the magnitude of each deep model's swing is partly an interaction with the Auto hyperparameter search: small universe changes shift the cross-validation surface enough that the Auto wrapper selects materially different configurations. Second, every panel evaluates the same univariate global setup without exogenous regressors. The robustness exercise does not turn on which cleaning convention is ``correct'' in any given case; the literature settles that question, often differently, in each one. It argues, rather, that any cross-paper comparison of forecasting methods that does not hold the cleaning recipe constant is at risk of confounding the comparison it intends to make. The Options panel adds a second warning: cleaning can flip not only the relative ranking of models but the sign of multiple deep architectures' $R^2_{\text{oos}}$, so even a coarse summary such as ``which model produces positive out-of-sample $R^2$'' is cleaning-dependent. Reproducible, version-controlled pipelines like FTSFR are what allow these comparisons to be made on fixed data, and they are what allow a reader to verify which side of each contrast they care about.
...
Second, the benchmark covers seven major asset classes (with both aggregated portfolio and disaggregated security-level data), five basis-spread datasets following \citet{Siriwardane2021}, and bank and intermediary indicators including bank Call Reports \citep{Drechsler2017} and the He--Kelly--Manela intermediary capital factors \citep{He2017}.

Third, we establish reproducible baselines using twelve forecasting models, ranging from classical statistical methods (ARIMA, Simple Exponential Smoothing, Theta) to hybrid linear architectures (DLinear, NLinear) and deep learning models (DeepAR, N-BEATS, N-HiTS, Vanilla Transformer, TiDE, KAN), all evaluated under one preprocessing pipeline and one rolling-origin cross-validation protocol. These baselines turn the benchmark into something researchers can build against rather than alongside.

Consistent with the M-competitions' lesson that no single method dominates universally, the empirical results show that model choice must respect the data-generating process, and that apparent disagreements across prior cross-study comparisons can be partly resolved once the data are held fixed. For asset returns, even the most sophisticated auto deep-learning architectures rarely beat the historical average: out-of-sample $R^2$ readings remain near zero across equities, corporate bonds, U.S.~Treasuries, FX, options, and CDS. We caveat this finding: it pertains to the class of models the benchmark evaluates, namely univariate global forecasts without exogenous regressors (see Section~\ref{subsec:univariate_multivariate}), and is consistent with decades of empirical asset-pricing evidence on how thin the exploitable signal is in that class. Extending the benchmark to methods that admit exogenous regressors and to other model classes (multivariate panels, ARIMAX, VARX, DeepAR with covariates, Temporal Fusion Transformers) is a natural follow-up and may sharpen what is forecastable in returns. The remaining two segments produce more nuanced rankings. On basis spreads, the deep architectures \texttt{NHITS}, \texttt{NLinear}, \texttt{NBEATS}, and \texttt{TiDE} all deliver materially more forecasting power than the historical-average baseline (median $R^2_{\text{oos}} > 0.64$, well clear of the best classical contender at $0.53$), exploiting persistent seasonal funding-quarter structure that classical univariate methods do not encode. On bank and intermediary indicators, by contrast, the family rankings are closer and depend on how performance is summarized: median $R^2_{\text{oos}}$ favors deep learning and classical roughly equally, while mean $R^2_{\text{oos}}$ favors classical because the classical baselines avoid the tail failures that occasionally hit the deep-learning aggregate. The simple classical methods (ARIMA, Theta) therefore remain a defensible default for bank-balance-sheet projection where worst-case behaviour matters, even though their typical performance is no better than the deep-learning competitors. The asset-pricing, risk-management, and financial-stability use cases for these forecasts therefore call for different default models, with the per-task rankings in Section~\ref{sec:results} guiding the choice.

Beyond the per-segment rankings, the sensitivity exercise in Section~\ref{subsec:cleaning_robustness} confirms the motivating thesis of the paper. Across four pairwise cleaning contrasts (TRACE-MMN signal cleaning, Constantinides-Jackwerth-Savov option-filter depth, NYSE-vs-CRSP breakpoints on Fama-French 25, and the Gürkaynak-Sack-Wright Treasury filter), the leaderboard reshuffles materially when only the cleaning convention changes. Classical baselines are stable across conventions, while deep models can swing by enough to flip the sign of $R^2_{\text{oos}}$ or move from top to below the historical-average baseline. The empirical takeaway is that ``which model works best?'' is a function of the cleaning recipe, and that reproducible, version-controlled pipelines are a precondition for any rigorous answer. We hope FTSFR provides a common benchmark against which future methodological progress in financial time series forecasting can be measured.
...
\begin{figure}[h]
  ...
  \end{tabular}
  \caption{Average returns of all stocks}
  \floatsource{Sources: Center for Research in Security Prices, Authors' analysis}
...
\begin{figure}[h]
  ...
  \end{tabular}
  \caption{Average three month rolling standard deviation}
  \floatsource{Sources: Center for Research in Security Prices, Authors' analysis}
...
\begin{figure}[h]
  ...
  \floatsource{Sources: Center for Research in Security Prices, He, Kelly, and Manela (2017), Authors' analysis}
...
\begin{figure}[h]
    ...
    \floatsource{Sources: Open Source Bond Asset Pricing, He, Kelly, and Manela (2017), Authors' analysis}
...
\begin{figure}[htbp]
  ...
  \end{tabular}
  \floatsource{Sources: OptionMetrics, Authors' analysis}
...
Bloomberg GSCI-based returns and the corresponding series in HKM. 
...
in HKM. Furthermore, HKM uses a different datasource than
...
\begin{figure}[htbp]
  ...
  \floatsource{Sources: Bloomberg, He, Kelly, and Manela (2017), Authors' analysis}
...
output frequency.  The cleaned bond returns panel is a monthly
...
bond prices directly from the S\&P Global bond-pricing tables and use the WRDS
...
daily S\&P Global bond pricing source alongside the \citet{Dickerson2024} filter;
...
no longer exactly offsets interest-rate differentials. Factors contributing to this phenomenon include heightened counterparty-credit risk or liquidity constraints.
...
\section{Forecasting Preprocessing Pipeline}
\label{app:forecasting_preprocessing}

Table~\ref{tab:filtered_dataset_stats} reports the impact of the unified preprocessing pipeline described in Section~\ref{sec:methodology} on each dataset in the benchmark. For every dataset we list the original and retained entity counts, the implied retention rate, median series lengths before and after filtering, and the adaptive minimum-observation threshold imposed by the length screen. The filtering rules are uniform across statistical and neural estimators so that every model sees the same vetted panel.
