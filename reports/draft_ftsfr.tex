...
Taken together, the cross-metric evidence confirms that performance is conditional: MASE-based rankings highlight the strength of NBEATS/NHITS on error scales commonly used by practitioners, while out-of-sample $R^2$ favors Theta and ARIMA when the economic question is whether we beat the historical mean. Analysts selecting a forecast should therefore align the evaluation metric with the economic decision. $R^2_{\text{oos}}$ is appropriate for return forecasting, where the historical mean is a credible trading benchmark. Scaled absolute errors are appropriate for arbitrage spreads, where seasonal accuracy matters more than mean-squared scorekeeping, and for bank and intermediary indicators, where parsimonious level forecasts capture most of the persistent dynamics.
