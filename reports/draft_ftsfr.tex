...
\caption*{\scriptsize \textbf{Note:} Each row aggregates every (model $\times$ dataset) observation whose model belongs to the indicated method type. ``\# Models'' counts distinct models in the type and ``N'' counts (model $\times$ dataset) observations. Lower MASE / Relative MASE indicates better performance; higher $R^2_{\text{oos}}$ indicates better performance. Best values within each panel (or, in Panel~B, within each dataset category) are bolded. The Classical row's Relative MASE excludes the Historic Average benchmark by construction. \emph{Sources: Bloomberg, Board of Governors of the Federal Reserve System, Center for Research in Security Prices, U.S. Call Reports, WRDS TRACE, OptionMetrics, S\&P Global, Authors' analysis.}}
\end{table}

The aggregated view sharpens the qualitative picture from the model-level tables. On the MASE scale the three families finish much closer than the individual rankings might suggest, with the hybrid and deep-learning aggregates trading the top spot for median MASE while classical models retain a slight edge on out-of-sample $R^2$. Panel~B shows that the gap between the families is overwhelmingly driven by the basis-spreads category: deep-learning and hybrid models exploit the seasonal structure of funding-related spreads and beat the classical aggregate by a non-trivial margin, while in the returns category every family clusters within a few hundredths of MASE and around zero $R^2_{\text{oos}}$. The takeaway is not that one family dominates uniformly, but that the value-add of moving from a classical baseline to a deep or hybrid architecture is concentrated in segments with persistent seasonal or cross-sectional structure, and is essentially absent in segments where the historical mean is already an oracle.

\subsection{Sensitivity to Cleaning Method}
\label{subsec:cleaning_sensitivity}

The motivating premise of FTSFR is that ``which forecasting method wins?'' is entangled with ``which cleaning recipe was applied?'' We close the results section by operationalizing this claim. We start with a deep-dive case study on corporate bonds, where a well-documented cleaning convention has a quantified effect on cross-sectional sorts (Section~\ref{subsubsec:mmn_sensitivity}). We then show that the same pattern recurs across three other asset classes (Section~\ref{subsubsec:cleaning_robustness}): options portfolios under different filtering depths, Fama-French 25 portfolios under NYSE vs. all-CRSP breakpoints, and Treasury bond portfolios under the Gurkaynak-Sack-Wright (2007) filters versus a permissive universe. In every demonstration, the underlying data come from the same source and the realized-return series is held fixed; only the cleaning convention applied during construction differs. Yet the forecasting leaderboard reshuffles materially, often by enough to flip the practitioner's choice of ``best'' model.

\subsubsection{A TRACE Case Study}
\label{subsubsec:mmn_sensitivity}
...
\subsubsection{Robustness Across Asset Classes}
\label{subsubsec:cleaning_robustness}
