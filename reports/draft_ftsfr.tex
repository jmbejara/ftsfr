...
\author{
        ...
        \and
        Zixiao Wang\thanks{Independent. Most of this work was completed as a student in the Financial Mathematics program at the University of Chicago}
        \and
...
\midrule
CDS Contract & Monthly returns for individual CDS contracts & \cite{He2017} \\
CDS Portfolio & Monthly returns for 20 CDS portfolios by tenor and credit quality & \cite{He2017} \\
...
SPX Options Contracts & Daily returns for individual SPX option contracts & \cite{Constantinides2013} \\
SPX Options Portfolios & Monthly returns for SPX option portfolios by moneyness & \cite{He2017} \\
HKM Monthly Factor & Monthly factor returns from He, Kelly, and Manela & \cite{He2017} \\
HKM Daily Factor & Daily factor returns from He, Kelly, and Manela & \cite{He2017} \\
...
\midrule
TIPS-Treasury & Monthly TIPS-Treasury basis spreads & Fed Yield Curve \\
Treasury-SF & Monthly Treasury-SF arbitrage spreads & Segmented Arbitrage \\
Treasury-Swap & Monthly Treasury-Swap arbitrage spreads & Segmented Arbitrage \\
CDS-Bond & Monthly CDS-bond basis spreads & Segmented Arbitrage \\
...
Bank Leverage & Quarterly leverage ratios from call report data & NYU Call Report \\
BHC Leverage & Quarterly bank holding company leverage ratios & NYU Call Report \\
Bank Cash Liquidity & Quarterly cash liquidity from call report data & NYU Call Report \\
BHC Cash Liquidity & Quarterly bank holding company cash liquidity & NYU Call Report \\
\bottomrule
\end{tabular}
\end{table}
...
\input{../_output/forecasting2/dataset_statistics.tex}

To ensure fair model comparisons, our forecasting system applies consistent filtering and data preprocessing across all models and datasets. Table~\ref{tab:filtered_dataset_stats} shows the impact of this filtering, displaying entity counts and median time series lengths before and after preprocessing. The filtering removes time series that are too short for reliable seasonal pattern detection and forecasting evaluation, while standardized data cleaning procedures handle missing values uniformly across all models. This preprocessing is essential for fair comparison between statistical and machine learning methods, as it ensures all models train and are evaluated on identical datasets. The filtering has varying impact across asset classes: portfolio-level datasets (which are typically well-maintained) show minimal changes, while disaggregated datasets show substantial filtering due to the presence of shorter-lived securities or incomplete data coverage.

\input{../_output/forecasting2/filtered_dataset_statistics.tex}
...
\input{../_output/forecasting2/mase_pivot_table.tex}
...
\input{../_output/forecasting2/relative_mase_pivot_table.tex}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{../_output/forecasting2/relative_mase_heatmap.png}
...
\input{../_output/forecasting2/rmse_pivot_table.tex}
...
comparable across the sample period. Finally, we reshape the wide-format panel 
into a long-format dataset of identifiers, dates, and returns, enabling seamless 
integration with the replication of \cite{He2017} factors.
...
Bloomberg GSCI-based returns and the corresponding series in \cite{Yang2013}. 
...
mismatches: the lists provided by \cite{Koijen2018} and \cite{Yang2013} do not 
perfectly overlap, and it is unclear which exact subset was ultimately adopted 
in \cite{He2017}. In other words, low-correlation pairs likely reflect incorrect 
matches rather than genuine return differences. The accompanying heatmap in 
Figure~\ref{fig:heatmap_gsci} highlights this heterogeneity visually, with 
clusters of high-correlation matches alongside a handful of clear mismatches.

\begin{figure}[H]
  \centering
  \begin{tabular}{@{}c@{}}
    \includegraphics[width=\linewidth,height=0.666\linewidth]{../docs_src/commod_return_comparison.png} \\[\abovecaptionskip]
  \end{tabular}
...
\begin{figure}[H]
  \centering
  \begin{tabular}{@{}c@{}}
    \includegraphics[width=\linewidth,height=0.666\linewidth]{../docs_src/commod_heatmap.png} \\[\abovecaptionskip]
  \end{tabular}
...
\input{../_output/forecasting2/smape_pivot_table.tex}
...
\input{../_output/forecasting2/mae_pivot_table.tex}
