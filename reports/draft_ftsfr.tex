...
To synthesize performance across datasets, Table~\ref{tab:median_mase_summary} reports median and mean statistics for MASE, Relative MASE, and out-of-sample $R^2$. Because MASE is the workhorse metric in the forecasting literature, we use it to anchor the discussion: \texttt{NBEATS} delivers the lowest median MASE, while \texttt{NHITS} dominates by mean MASE. Both auto-deep models handily outperform classical baselines such as the Historic Average, the seasonal naïve benchmark, and simple exponential smoothing (all of which sit well above 1.0 on Relative MASE). Their automated hyperparameter search concentrates capacity on recurring seasonal windows, which is particularly valuable for the high-frequency panels in our benchmark. When we pivot to $R^2_{\text{oos}}$, however, the ranking changes: \texttt{ARIMA} produces both the strongest median (0.352) and the strongest mean (0.263), with the simple linear deep networks \texttt{NLinear} (0.343) and \texttt{DLinear} (0.307) close behind on the median. The classical specification and the lightweight linear deep architectures together deliver the largest reductions in mean-squared error relative to the historical mean benchmark, while the heavier auto-deep models that win on MASE (NBEATS, NHITS) post weaker median $R^2_{\text{oos}}$ (0.098 and 0.277) and slightly negative means.
...
The aggregated view sharpens the qualitative picture from the model-level tables. On the MASE scale the three method families finish much closer than the individual rankings might suggest: hybrid wins median MASE (0.836), with deep learning and classical within a few hundredths (0.858 and 0.892 respectively). The out-of-sample $R^2$ column tells a more interesting story. Hybrid posts the best \emph{median} $R^2_{\text{oos}}$ (0.325), but its \emph{mean} is sharply negative ($-0.880$), reflecting catastrophic losses on a handful of datasets. Deep learning shows a much milder version of the same pattern (median $0.192$, mean $-0.020$). Classical statistical methods post the only solidly positive mean $R^2_{\text{oos}}$ ($+0.060$), because they avoid the tail failures even though their typical performance is unremarkable. Panel~B shows that the family gap is concentrated in the basis-spreads category, where hybrid and deep-learning models exploit the seasonal structure of funding-related spreads and beat the classical aggregate on median MASE (0.442 and 0.479 vs.\ 0.691). On the Returns category every family clusters within a few hundredths of MASE and around zero $R^2_{\text{oos}}$. The Other category (bank regulatory filings and intermediary indicators) actually flips the basis-spreads ranking: classical statistical methods post the highest median and mean $R^2_{\text{oos}}$ (0.462 and 0.470) in that segment, edging out hybrid (0.380, 0.372) and deep learning (0.436, $0.310$). The takeaway is not that one family dominates uniformly. Rather, the value-add of moving from a classical method to a deep or hybrid architecture is concentrated in segments with persistent seasonal or cross-sectional structure (basis spreads), is essentially absent in segments where the historical mean is already an oracle (returns), and can reverse on segments where parsimony pays off (bank and intermediary indicators).
