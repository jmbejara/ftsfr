...
To synthesize performance across datasets, Table~\ref{tab:median_mase_summary} reports median and mean statistics for MASE and out-of-sample $R^2$. Because MASE is the workhorse metric in the forecasting literature, we use it to anchor the discussion: \texttt{DLinear} delivers the lowest median MASE (0.755), with \texttt{NHITS} (0.767), \texttt{ARIMA} (0.772), \texttt{NBEATS} (0.775), and \texttt{NLinear} (0.778) within a few hundredths. The auto-deep models comfortably beat the Historic Average (0.873) and simple exponential smoothing (0.874), though the classical \texttt{ARIMA} and \texttt{Theta} (0.810) remain firmly in the leading pack. When we pivot to $R^2_{\text{oos}}$, the magnitudes shrink dramatically: \emph{every} model in the suite, including the classical baselines, posts a median $R^2_{\text{oos}}$ within $0.04$ of zero. Mean $R^2_{\text{oos}}$ is modestly positive for most of the suite (\texttt{Transformer} 0.171, \texttt{Theta} 0.155, \texttt{ARIMA} 0.142, \texttt{DLinear} 0.118), with \texttt{NBEATS} ($-0.264$) the one clear laggard---an artifact of a single small portfolio panel on which it misjudges the level of a near-constant series. The takeaway from the pivot is that an aggregate $R^2_{\text{oos}}$ near zero is the rule rather than the exception in this univariate benchmark: the historical-average benchmark itself is already the right forecaster for the typical panel, and gains on MASE come from sharpening the level forecast rather than from extracting time-series predictability that would translate into a large positive $R^2_{\text{oos}}$.
...
Basis spreads tell a different story. On median MASE the \texttt{Transformer} leads (0.400), with the classical \texttt{Theta} (0.411), \texttt{KAN} (0.444), \texttt{ARIMA} (0.446), and \texttt{NLinear} (0.452) clustered closely behind. The deep architectures excel here because they decompose the series into trend and seasonal components while borrowing strength across entities through global training. Basis spreads exhibit pronounced seasonal structure (e.g., quarter-end funding pressure) and medium-term mean reversion, and the auto wrappers concentrate the search on those horizons. The separation appears on $R^2_{\text{oos}}$: the deep and hybrid models (NLinear, NBEATS, TiDE, NHITS) all post median $R^2_{\text{oos}}$ of 0.61--0.63, ahead of the best classical contenders (\texttt{Theta} at 0.59, \texttt{ARIMA} at 0.57), and \texttt{NBEATS} posts the strongest mean (0.65). The modern architectures thus deliver large improvements over the Historic Average and simple exponential smoothing baselines and a consistent, if not overwhelming, edge over classical methods on these spreads.

The ``Other" category, which includes bank regulatory filings, volatility indicators, and macro-financial composites, is more nuanced. Median MASE is tightly bunched: \texttt{NHITS} (0.772), \texttt{DLinear} (0.775), \texttt{NBEATS} (0.776), and the classical \texttt{Theta} (0.784) sit within roughly a hundredth of one another. On $R^2_{\text{oos}}$ the hybrid and lighter deep models hold a modest edge---medians of 0.10--0.16 for \texttt{KAN}, \texttt{TiDE}, \texttt{NLinear}, and \texttt{DLinear} versus 0.03 for \texttt{Theta} and \texttt{ARIMA}---and mean $R^2_{\text{oos}}$ is solidly positive for most of the suite (\texttt{DLinear} 0.34, \texttt{Transformer} 0.33, \texttt{NLinear} 0.29), with \texttt{DeepAR} the laggard. These datasets are typically longer and smoother than the high-volatility returns series but lack the strong seasonal spikes of basis spreads, so parsimonious trend models and the lighter decomposition-based architectures both fit naturally, and the gap between classical and modern methods is correspondingly small.

Taken together, the cross-metric evidence confirms that performance is conditional: MASE-based rankings highlight the strength of the decomposition-based architectures (DLinear, NHITS, NBEATS) on error scales commonly used by practitioners, while out-of-sample $R^2$ shows most methods beating the historical mean on average across the benchmark---with essentially all of that advantage earned on basis spreads and the smoother bank and intermediary indicators rather than on returns. Analysts selecting a forecast should therefore align the evaluation metric with the economic decision. $R^2_{\text{oos}}$ is appropriate for return forecasting, where the historical mean is a credible trading benchmark. Scaled absolute errors are appropriate for arbitrage spreads, where seasonal accuracy matters more than mean-squared scorekeeping, and for bank and intermediary indicators, where parsimonious level forecasts capture most of the persistent dynamics.
...
The aggregated view sharpens the qualitative picture from the model-level tables. On the MASE scale the three method families finish much closer than the individual rankings might suggest: hybrid wins median MASE (0.766), with deep learning (0.784) and classical (0.814) within a few hundredths, and all three comfortably ahead of the benchmark (0.873). On out-of-sample $R^2$, every family posts a \emph{positive} overall mean (classical $0.132$, hybrid $0.045$, deep learning $0.024$) with medians within $0.03$ of zero---no family systematically loses to the historical average once forecasts are stabilized as described in Section~\ref{sec:methodology}. Panel~B shows where that aggregate advantage is earned. In the basis-spreads category, hybrid and deep-learning models exploit the seasonal structure of funding-related spreads and beat the classical aggregate on median MASE (0.465 and 0.492 vs.\ 0.691), with mean $R^2_{\text{oos}}$ of 0.55 and 0.54 (vs.\ 0.43). On the Returns category every family clusters near the benchmark on MASE---deep learning posts the best median (0.726)---while median $R^2_{\text{oos}}$ sits at roughly $-0.03$ for all three families: relative to the historical mean, returns remain essentially unforecastable. The remaining negative \emph{mean} $R^2_{\text{oos}}$ values on returns (classical $-0.08$, deep learning $-0.26$, hybrid $-0.35$) trace to a handful of small, low-volatility portfolio panels on which an in-range level bias is heavily punished by the squared-error metric; the median, which is robust to those cases, is the more informative summary. The Other category (bank regulatory filings and intermediary indicators) modestly favors the hybrid family on both median MASE (0.786) and $R^2_{\text{oos}}$ (median 0.097, mean 0.318), with classical methods close behind on the mean (0.264) and deep learning trailing (0.124). The takeaway is not that one family dominates uniformly. Rather, the value-add of moving from a classical method to a deep or hybrid architecture is concentrated in segments with persistent seasonal or cross-sectional structure (basis spreads), is essentially absent in segments where the historical mean is already an oracle (returns), and is modest in segments where parsimony pays off (bank and intermediary indicators).
...
The Options panel (Panel~A) is the widest contrast and reads mostly as a level shift with a reordered upper tier. Moving from the permissive L1 panel to the full L3 stack raises MASE for every model, the benchmark included, and lowers $R^2_{\text{oos}}$ for ten of the twelve models; only the sequence models DeepAR ($+0.10$) and NBEATS ($+0.02$) gain. \texttt{TiDE} posts the best $R^2_{\text{oos}}$ on both variants (0.28 and 0.21), but the best-MASE model flips from DLinear on L1 to TiDE on L3, and three architectures cross zero $R^2_{\text{oos}}$ (ARIMA, DeepAR, Transformer). The heavier L3 cleaning is also the defensible convention rather than a stylistic preference: its filters drop deep out-of-the-money and far-from-maturity options with bid--ask spreads exceeding 50\% of price and near-zero volume, quotes no market maker would fill at scale, so the L1 panel admits returns no investor could realise even in principle.

The FF25 panel (Panel~B) sits in the middle. Every $R^2_{\text{oos}}$ is negative on both variants, consistent with the broader equity-return result in Section~\ref{sec:results}, so there is no leaderboard winner to flip; but the deep models still swing materially and in mixed directions (DeepAR by $-0.31$, DLinear by $+0.17$, NHITS by $+0.16$, Transformer by $-0.13$, KAN by $-0.12$) while the classical baselines move by at most $0.10$ (SES and Theta by just $0.01$). The magnitudes are large enough that a paper claiming a particular deep model ``works on FF25'' could see its claim reversed by switching breakpoint conventions.

The Treasury panel (Panel~C) is the closest analogue to the MMN setting: the two universes are 99\%+ correlated within bucket, yet the ranking still reorders. The classical baselines barely move ($|\Delta R^2_{\text{oos}}| \leq 0.014$), with ARIMA second under both filter choices, while the top spot flips between deep architectures---Transformer leads the permissive universe ($0.111$) but NBEATS leads the strict one ($0.132$)---and the large swings live entirely among the deep models: DeepAR falls by $0.22$ while NHITS ($+0.22$), NBEATS ($+0.17$), DLinear ($+0.10$), and NLinear ($+0.08$) move the other way, with four architectures crossing zero. As in the TRACE case, essentially all of the ranking volatility lives among the deep models, with classical baselines essentially stable.

Two qualifications carry over. First, the magnitude of each deep model's swing is partly an interaction with the Auto hyperparameter search: small universe changes shift the cross-validation surface enough that the Auto wrapper selects materially different configurations. Second, every panel evaluates the same univariate global setup without exogenous regressors. The robustness exercise does not turn on which cleaning convention is ``correct'' in any given case; the literature settles that question, often differently, in each one. It argues, rather, that any cross-paper comparison of forecasting methods that does not hold the cleaning recipe constant is at risk of confounding the comparison it intends to make. The Options and Treasury panels add a second warning: cleaning can flip not only the relative ranking of models but the sign of several architectures' $R^2_{\text{oos}}$, so even a coarse summary such as ``which model produces positive out-of-sample $R^2$'' is cleaning-dependent. Reproducible, version-controlled pipelines like FTSFR are what allow these comparisons to be made on fixed data, and they are what allow a reader to verify which side of each contrast they care about.
