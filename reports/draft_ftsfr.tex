...
\begin{figure}[h]
  ...
  \begin{tabular}{@{}c@{}}
    \includegraphics[width=.95\linewidth]{\PathToAssets/CDS_replicate.png}
  \end{tabular}
...
Our implementation draws bond observations from the Open Source Bond Asset Pricing panel of \citet{Dickerson2023} and CDS quotes from Markit via WRDS, restricting to USD-denominated senior unsecured contracts on US firms at the 1-, 3-, 5-, 7-, and 10-year tenors and joining the two sides through the RED CUSIP-to-Markit mapping table. The bond filters follow the source paper: senior unsecured USD debt with a valid ISIN, fixed coupon, one- to ten-year remaining maturity, outstanding principal of at least \$100{,}000, and we exclude putable bonds, convertible bonds, and bonds trading below half of par. The most consequential deviation from the published methodology concerns the floating-rate-spread input: the Z-spread field is not populated in our Markit extract, so we substitute a duration-matched credit spread (the bond yield less the yield on a Treasury portfolio matched to the bond's modified duration) and flag this as a known source of measurement difference relative to a Markit Z-spread construction. We require at least two distinct CDS tenors per firm-date for the cubic-spline interpolation and discard firm-dates that fail this requirement rather than extrapolating, and we drop observations whose implied risk-free rate exceeds 100 percent in absolute value.
