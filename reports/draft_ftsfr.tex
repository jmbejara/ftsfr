...
The Options panel shows the same shape of lesson as the MMN case study---a reshuffled leaderboard rather than a uniform shift. No model achieves $R^2_{\text{oos}}$ above $+0.21$ on either panel, and the typical model lands in $[-0.5, +0.2]$ on both. Within that band, the leaderboard reorders sharply between L1 and L3, and in mixed directions: five neural architectures (NHITS, NBEATS, KAN, TiDE, Transformer) gain from the heavier L3 cleaning by $\Delta R^2_{\text{oos}}$ between $+0.17$ and $+0.66$, while three (NLinear, DeepAR, DLinear) and three of the four classical baselines lose by $-0.02$ to $-0.15$. Two of those neural swings also cross zero ($R^2_{\text{oos}}$ moves from negative on L1 to positive on L3 for NHITS and Transformer); KAN and NBEATS gain materially but remain negative on both panels. The top of the L1 leaderboard is NLinear on $R^2_{\text{oos}}$ ($+0.173$) and DeepAR on MASE ($0.561$); the top of the L3 leaderboard is TiDE on both ($R^2_{\text{oos}} = +0.203$, MASE $= 0.742$). The four classical baselines move by $0.15$ or less in $R^2_{\text{oos}}$, so---as in every other panel of this exercise---the cleaning-sensitivity volatility lives in the deep models.

Independent of the forecasting numbers, the L3 cleaning is the right convention rather than a stylistic preference. The Level-2 filters that produce the L3 panel drop quotes with implied volatility outside $[0.05, 1.00]$, moneyness outside $[0.8, 1.2]$, and put--call-parity violations; the Level-3 filters then drop IV-curve and PCP outliers. The contracts removed by these filters are overwhelmingly deep out-of-the-money or far-from-maturity options for which bid--ask spreads can exceed 50\% of the quoted price, daily trading volume is zero or near-zero, and OptionMetrics records a settlement quote that no market maker would actually fill at scale. The L1 panel therefore admits returns that no investor could realise even in principle, regardless of what the forecasting metrics say. Panel~A then quantifies a different question---whether the same model suite run on a clean vs.\ permissively-cleaned universe gives the same ranking---and the answer is that it does not: the leaderboard top moves from NLinear/DeepAR on L1 to TiDE on L3, seven of eight deep architectures shift by $|\Delta R^2_{\text{oos}}| \geq 0.13$, and two cross zero. As in the MMN setting, a cross-paper comparison that does not hold the cleaning recipe constant risks confounding cleaning skill with method skill, even when (as here) every $R^2_{\text{oos}}$ stays inside $[-0.7, +0.2]$ on both sides.

The FF25 panel sits in the middle. Almost every $R^2_{\text{oos}}$ value is negative---NLinear is the only model that achieves a positive (if small) $R^2_{\text{oos}}$ on either variant ($+0.059$ on NYSE breakpoints, $+0.026$ on CRSP breakpoints)---consistent with the broader equity-return result documented in Section~\ref{sec:results}. But the deep models still swing materially between conventions: Transformer by $-0.441$ (the largest swing in the panel), NBEATS by $-0.281$, DeepAR by $+0.204$, KAN by $-0.177$, TiDE by $-0.139$. Classical baselines are essentially unchanged ($|\Delta R^2_{\text{oos}}| \leq 0.03$ for ARIMA, SES, and Theta), so the cleaning sensitivity is concentrated in exactly the model class that practitioners reach for when looking for predictive lift. No model dominates either variant, so the leaderboard-flip question is moot here, but the magnitudes are large enough that a paper claiming any particular deep model ``works on FF25'' could see its claim reversed by switching breakpoint conventions.
