...
To synthesize performance across datasets, Table~\ref{tab:median_mase_summary} reports median and mean statistics for MASE, Relative MASE, and out-of-sample $R^2$. Because MASE is the workhorse metric in the forecasting literature, we use it to anchor the discussion: \texttt{NBEATS} delivers the lowest median MASE (0.779), with \texttt{NHITS} (0.806), \texttt{TiDE} (0.806), and \texttt{DLinear} (0.790) within a few hundredths. All of these auto-deep models handily outperform classical baselines such as the Historic Average, the seasonal naïve benchmark, and simple exponential smoothing (all of which sit well above 1.0 on Relative MASE). Their automated hyperparameter search concentrates capacity on recurring seasonal windows, which is particularly valuable for the high-frequency panels in our benchmark. When we pivot to $R^2_{\text{oos}}$, however, the magnitudes shrink dramatically: \emph{every} model in the suite, including the classical baselines, posts a median $R^2_{\text{oos}}$ within $\pm 0.03$ of zero. \texttt{ARIMA} produces the strongest mean (0.159), with \texttt{TiDE} (0.098) the only other model in solidly positive territory; among the rest, \texttt{Theta} and \texttt{KAN} sit near $-0.13$ to $-0.14$ mean while \texttt{NBEATS} ($-0.486$) is the clear laggard. The takeaway from the pivot is that an aggregate $R^2_{\text{oos}}$ near zero is the rule rather than the exception in this univariate benchmark: the historical-average benchmark itself is already the right forecaster for the typical panel, and gains on MASE come from sharpening the level forecast rather than from extracting time-series predictability that would translate into a positive $R^2_{\text{oos}}$.
...
The Options panel shows the same shape of lesson as the MMN case study---a reshuffled leaderboard rather than a uniform shift. No model achieves $R^2_{\text{oos}}$ above $+0.27$ on either panel, and the typical model lands in $[-0.7, +0.2]$ on both (with NBEATS the lone outlier on L1 at $R^2_{\text{oos}} = -1.03$). Within that band, the leaderboard reorders sharply between L1 and L3, and in mixed directions: three neural architectures (NBEATS, NHITS, KAN) gain from the heavier L3 cleaning by $\Delta R^2_{\text{oos}}$ between $+0.41$ and $+0.79$, while five deep models (NLinear, TiDE, DLinear, Transformer, DeepAR) and three of the four classical baselines lose by $-0.11$ to $-0.68$. Two of the neural swings cross zero from below ($R^2_{\text{oos}}$ moves from negative on L1 to positive on L3 for NHITS and KAN), and Transformer crosses zero in the opposite direction (L1 $+0.17 \to$ L3 $-0.01$); NBEATS gains materially but remains negative on both panels. The top of the L1 leaderboard is DLinear on $R^2_{\text{oos}}$ ($+0.208$) and DeepAR on MASE ($0.575$); the top of the L3 leaderboard is NHITS on both ($R^2_{\text{oos}} = +0.262$, MASE $= 0.749$). The four classical baselines move by $0.15$ or less in $R^2_{\text{oos}}$, so---as in every other panel of this exercise---the cleaning-sensitivity volatility lives in the deep models.

Independent of the forecasting numbers, the L3 cleaning is the right convention rather than a stylistic preference. The Level-2 filters that produce the L3 panel drop quotes with implied volatility outside $[0.05, 1.00]$, moneyness outside $[0.8, 1.2]$, and put--call-parity violations; the Level-3 filters then drop IV-curve and PCP outliers. The contracts removed by these filters are overwhelmingly deep out-of-the-money or far-from-maturity options for which bid--ask spreads can exceed 50\% of the quoted price, daily trading volume is zero or near-zero, and OptionMetrics records a settlement quote that no market maker would actually fill at scale. The L1 panel therefore admits returns that no investor could realise even in principle, regardless of what the forecasting metrics say. Panel~A then quantifies a different question---whether the same model suite run on a clean vs.\ permissively-cleaned universe gives the same ranking---and the answer is that it does not: the leaderboard top moves from DLinear/DeepAR on L1 to NHITS on L3, every one of the eight deep architectures shifts by $|\Delta R^2_{\text{oos}}| > 0.11$ (six of them by more than $0.20$), and three cross zero. As in the MMN setting, a cross-paper comparison that does not hold the cleaning recipe constant risks confounding cleaning skill with method skill.

The FF25 panel sits in the middle. Almost every $R^2_{\text{oos}}$ value is negative---NLinear is the only model that achieves a positive (if small) $R^2_{\text{oos}}$ on either variant ($+0.059$ on NYSE breakpoints, $+0.026$ on CRSP breakpoints)---consistent with the broader equity-return result documented in Section~\ref{sec:results}. But the deep models still swing materially between conventions: Transformer by $-0.441$ (the largest swing in the panel), NBEATS by $-0.281$, DeepAR by $+0.204$, KAN by $-0.177$, TiDE by $-0.139$. Classical baselines are essentially unchanged ($|\Delta R^2_{\text{oos}}| \leq 0.04$ for ARIMA, SES, and Theta), so the cleaning sensitivity is concentrated in exactly the model class that practitioners reach for when looking for predictive lift. No model dominates either variant, so the leaderboard-flip question is moot here, but the magnitudes are large enough that a paper claiming any particular deep model ``works on FF25'' could see its claim reversed by switching breakpoint conventions.

The Treasury panel is the closest analogue to the MMN setting: the two universes are 99\%+ correlated within bucket, yet the leaderboard reorders sharply. The classical baselines (ARIMA $0.277 \to 0.259$, Theta $0.259 \to 0.250$, SES $0.230 \to 0.225$) hold their position at the top under both filter choices and barely move ($|\Delta R^2_{\text{oos}}| \leq 0.02$). The deep models are the ones that swing. TiDE is near the top of permissive on $R^2_{\text{oos}}$ ($0.272$, second only to ARIMA's $0.277$), but collapses on GSW-strict ($R^2_{\text{oos}} = -0.043$, $\Delta = -0.315$). NBEATS makes the largest fall in the panel: $\Delta R^2_{\text{oos}} = -0.375$, moving from competitive on permissive ($+0.213$) to negative on strict ($-0.162$). Going the other direction, NHITS and Transformer recover materially from the strict cleaning ($\Delta R^2_{\text{oos}} = +0.261$ and $+0.202$ respectively), though both stay negative on both panels. DeepAR moves $-0.211$. As in the TRACE case, the entire ranking volatility lives among the deep models, with classical baselines essentially stable.

The short-term-reversal panel (Panel D) brings the cleaning-sensitivity argument back to its TRACE origin and adds a fourth failure mode of its own. The portfolio panels here are the same data underlying Table~\ref{tab:str_dickerson}: the naive variant embeds the Dickerson CEIV bias, the return-gap variant does not. The forecasting result is messier than Panels A--C. Most models post negative $R^2_{\text{oos}}$ on both variants---only DLinear achieves a positive value on either side ($+0.360$ on the naive panel, $+0.110$ on the corrected panel), and even DLinear is itself cleaning-sensitive ($\Delta R^2_{\text{oos}} = -0.249$). Among the models that swing most under correction, KAN moves dramatically toward zero ($-3.823 \to -0.477$, $\Delta = +3.346$, the largest swing in the entire stacked table) and NBEATS recovers from $-1.218 \to -0.793$ ($\Delta = +0.425$). Going the other direction, NHITS, NLinear, and TiDE all get \emph{worse} on the corrected panel ($\Delta R^2_{\text{oos}}$ of $-1.387$, $-0.426$, and $-0.263$ respectively). Classical baselines barely move ($|\Delta R^2_{\text{oos}}| \leq 0.16$). The implication aligns with Panels A--C: deep models trained on TRACE-derived signals can react to the cleaning convention in opposite directions, and a researcher who picks a model under one convention has no guarantee that the same model ranks similarly under another.
...
Beyond the per-segment rankings, the sensitivity exercise in Section~\ref{subsec:cleaning_robustness} confirms the motivating thesis of the paper. Across five pairwise cleaning contrasts (TRACE-MMN signal cleaning, Constantinides-Jackwerth-Savov option-filter depth, NYSE-vs-CRSP breakpoints on Fama-French 25, the Gürkaynak-Sack-Wright Treasury filter, and the Dickerson short-term-reversal correction), the leaderboard reshuffles materially when only the cleaning convention changes. Classical baselines are stable across conventions, while deep models can swing by enough to flip the sign of $R^2_{\text{oos}}$ or move from top to below the historical-average baseline. The empirical takeaway is that ``which model works best?'' is a function of the cleaning recipe, and that reproducible, version-controlled pipelines are a precondition for any rigorous answer. We hope FTSFR provides a common benchmark against which future methodological progress in financial time series forecasting can be measured.
