...
\author{
        Jeremiah Bejarano\thanks{Office of Financial Research, U.S. Department of the Treasury} \footnote{All mistakes are my own. Views and opinions expressed are those of the authors and do not necessarily represent official positions or policy of the Office of Financial Research (OFR) or the U.S. Department of the Treasury. Please address correspondence to \href{mailto:jeremiah.bejarano@ofr.treasury.gov}{jeremiah.bejarano@ofr.treasury.gov}.} 
        \and
        ...
        \and
        Alex Zixiao Wang\thanks{Independent. A significant portion of this work was completed as students in the Financial Mathematics program at the University of Chicago}
        \and
...
The volume of SPX options traded has grown dramatically over time (along with the options asset class as a whole). Our options dataset and code module provides data filtration and portfolio construction functions following the procedures outlined in \cite{Constantinides2013} ("CJS"), and the aggregation of these portfolio returns into the 18 SPX option portfolios utilized in \cite{He2017} ("HKM").

\begin{figure}[h!]
  \centering
  \begin{tabular}{@{}c@{}}
    \includegraphics[width=.7\linewidth,height=200pt,width=400pt]{../docs_src/spx_options_over_time.png}
  \end{tabular}
  \caption{Volume of Options in the Raw SPX Data}
  \label{fig:spx_options_volume}
\end{figure}

Our options dataset provides monthly leverage-adjusted portfolio returns for SPX option portfolios, spanning 23 years from January 1996 to December 2019. The monthly returns were constructed from the daily data of 18.9 million individual SPX option contracts. 

The leverage adjustments and dynamic portfolio rebalancing process outlined in CJS had the overarching objective of constructing monthly portfolio returns that were roughly normally distributed over time, and only moderately skewed. We document our good faith reproduction of these procedures, and acknowledge areas where we made educated guesses about the authors' intent, if a particular process was not sufficiently detailed in the original paper. We provide the user with generalized functions that operate on **any set** of options data that is structured the same as the dataset we utilized (WRDS OptionMetrics). This should hopefully require no more than a few simple data transformations of standardized financial metrics if the user is utilizing a source of option pricing data other than WRDS OptionMetrics.
...
The CJS portfolios are organized by option type (call or put), moneyness (9 levels), and maturity (3 levels), leading to $2 \times 9 \times 3 = 54$ distinct portfolios. The 18 HKM portfolios were constructed by taking an equal weight average across the 3 maturities of the CJS portfolios, so we have $2 \times 9 = 18$ distinct portfolios in the HKM framework. We have provided a simple coding system to access and identify these portfolios.

Additionally, CJS utilized a 3-level data filtration system to clean the raw option quotes. In replicating their work, we found it helpful to create some "flat time" visualizations of the entire dataset, in order to see the data filters in action. These visualizations are integrated into the filtration code, so will be generated when our filtration functions are utilized.

For our SPX options data, this is what each level of filter did to the raw set of options (each dot / point on these charts represents a single option in the raw data):

\begin{figure}[h!]
  \centering
  \begin{tabular}{@{}c@{}}
    \includegraphics[width=.7\linewidth,height=200pt,width=400pt]{../docs_src/RAW_1996-01_2019-12_iv_summary.png}
  \end{tabular}
  \caption{Some visualizations of the raw SPX options data}
...
\begin{figure}[h!]
  \centering
  \begin{tabular}{@{}c@{}}
    \includegraphics[width=.7\linewidth,height=200pt,width=400pt]{../docs_src/L1_1996-01_2019-12_iv_summary.png}
  \end{tabular}
  \caption{SPX options remaining after Level 1 filters}
...
\begin{figure}[h!]
  \centering
  \begin{tabular}{@{}c@{}}
    \includegraphics[width=.7\linewidth,height=200pt,width=400pt]{../docs_src/L2_1996-01_2019-12_iv_summary.png}
  \end{tabular}
...
\begin{figure}[h!]
  \centering
  \begin{tabular}{@{}c@{}}
    \includegraphics[width=.7\linewidth,height=200pt,width=400pt]{../docs_src/L3_IV_1996-01_2019-12_iv_summary.png}
  \end{tabular}
...
\begin{figure}[h!]
  \centering
  \begin{tabular}{@{}c@{}}
    \includegraphics[width=.7\linewidth,height=200pt,width=400pt]{../docs_src/L3_IV_PCP_1996-01_2019-12_iv_summary.png}
  \end{tabular}
  \caption{SPX options remaining after the Level 3 Implied Volatility and Put-Call Parity filters}
